...
当前参考构建下，安装包约 `6.47 MB`(按 Android 系统千进制换算)，这些数字会随版本、ABI 与构建配置变化。
...
FlipBits 图标资源（包括源文件与组件 SVG 文件）作为公共资源向社区开放。

在遵守本仓库许可证与适用法律的前提下，您可以将这些图标自由用于徽章、二创衍生、视频展示与商业发布（包括售卖），无需额外申请单独授权。

**建议署名：**
`图标来源：github.com/camellia2077/FlipBits`
...
- **附加说明文件**：更详细的分发声明与商标边界说明见 [NOTICE](NOTICE) 与 [docs/legal/TRADEMARKS.md](docs/legal/TRADEMARKS.md)。
