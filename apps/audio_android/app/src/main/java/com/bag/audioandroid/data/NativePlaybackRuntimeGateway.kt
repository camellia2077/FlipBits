...
class NativePlaybackRuntimeGateway : PlaybackRuntimeGateway {
    ...
    override fun load(totalSamples: Int, sampleRateHz: Int): PlaybackUiState =
        decodeState(NativePlaybackRuntimeBridge.nativeLoadState(totalSamples, sampleRateHz))
    ...
    override fun paused(state: PlaybackUiState): PlaybackUiState =
        decodeState(NativePlaybackRuntimeBridge.nativePaused(encodeState(state)))
    ...
    override fun progress(state: PlaybackUiState, playedSamples: Int): PlaybackUiState =
        decodeState(NativePlaybackRuntimeBridge.nativeProgress(encodeState(state), playedSamples))
    ...
    override fun scrubChanged(state: PlaybackUiState, targetSamples: Int): PlaybackUiState =
        decodeState(NativePlaybackRuntimeBridge.nativeScrubChanged(encodeState(state), targetSamples))
    ...
    override fun failed(state: PlaybackUiState): PlaybackUiState =
        decodeState(NativePlaybackRuntimeBridge.nativeFailed(encodeState(state)))

    override fun clampSamples(totalSamples: Int, sampleIndex: Int): Int =
        NativePlaybackRuntimeBridge.nativeClampSamples(totalSamples, sampleIndex)

    override fun fractionToSamples(totalSamples: Int, fraction: Float): Int =
        NativePlaybackRuntimeBridge.nativeFractionToSamples(totalSamples, fraction)

    override fun progressFraction(state: PlaybackUiState): Float =
        NativePlaybackRuntimeBridge.nativeProgressFraction(encodeState(state))

    override fun elapsedMs(state: PlaybackUiState): Long =
        NativePlaybackRuntimeBridge.nativeElapsedMs(encodeState(state))

    override fun totalMs(state: PlaybackUiState): Long =
        NativePlaybackRuntimeBridge.nativeTotalMs(encodeState(state))

    private fun encodeState(state: PlaybackUiState): IntArray = intArrayOf(
    ...
    private fun decodeState(rawState: IntArray): PlaybackUiState {
        val values = if (rawState.size >= PLAYBACK_STATE_FIELD_COUNT) {
            rawState
        } else {
