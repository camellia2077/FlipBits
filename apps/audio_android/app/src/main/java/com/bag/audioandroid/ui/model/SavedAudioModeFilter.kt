...
enum class SavedAudioModeFilter(
    ...
    fun matches(item: SavedAudioItem): Boolean = mode == null || item.modeWireName == mode.wireName

    companion object {
        fun fromTransportMode(mode: TransportModeOption): SavedAudioModeFilter = entries.firstOrNull { it.mode == mode } ?: All
