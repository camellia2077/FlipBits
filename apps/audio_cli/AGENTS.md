...
- 若改动涉及共享业务逻辑或跨边界接线，优先读这些文档，不要先全量 `rg`：
  ...
  - `docs/history/presentation/cli/v0.2/0.2.8.md`

## 快速定位

- 优先按职责找入口，不要默认从最大文件或全仓搜索开始。
- 常见入口速查：
  - CLI 参数与子命令定义：`rust/src/presentation/cli.rs`
  - 命令分发与 use case：`rust/src/commands/mod.rs`、`rust/src/commands/encode.rs`、`rust/src/commands/decode.rs`
  - CLI metadata policy：`rust/src/commands/metadata.rs`
  - `bag_api` FFI 封装：`rust/src/bag_api/`
  - `audio_io` WAV / metadata FFI 封装：`rust/src/audio_io_api/`
  - 文本 / 文件读写：`rust/src/presentation/fs_io.rs`
  - 进度条等终端表现逻辑：`rust/src/presentation/progress.rs`
  - 输出与错误类型：`rust/src/presentation/output.rs`、`rust/src/presentation/error.rs`
...
- 改命令参数、帮助文案、子命令结构：
  - `rust/src/presentation/cli.rs`
  - `rust/src/tests.rs`
  - `rust/tests/cli.rs`
- 改 encode/decode 行为、输出文案、stdout/stderr 契约：
  - `rust/src/commands/`
  - `rust/src/presentation/output.rs`
  - `rust/src/presentation/error.rs`
  - `rust/tests/cli.rs`
- 改与 `bag_api` 的接线：
  - `rust/src/bag_api/`
...
- 改终端进度条或交互式输出行为：
  - `rust/src/presentation/progress.rs`
  - `rust/src/presentation/output.rs`
  - `rust/src/commands/encode.rs`
- 改文件输入输出与 UTF-8 文本读取：
  - `rust/src/presentation/fs_io.rs`
- 改构建、链接、静态库搜索路径：
  - `rust/build.rs`
...
- 改 FFI 结构体、enum 或 ABI 常量时，必须同步对齐：
  - Rust 侧 `rust/src/bag_api/` / `rust/src/audio_io_api/`
  - C/C++ 侧头文件与实现
...
- 需要先整理 Rust 代码格式时，优先运行：
  - `cargo fmt --manifest-path apps/audio_cli/rust/Cargo.toml`
...
- 若只想检查真实 CLI 子进程行为，可定向运行：
  - 优先仍运行 `python tools/run.py cli test`，保证 GNU target、native libraries 和 Cargo 环境与仓库入口一致
  - 确需只跑集成测试时，先确认 `build/dev` native libraries 已构建，再在 `apps/audio_cli/rust/` 下使用仓库支持的 `x86_64-pc-windows-gnu` target
