...
3. `NativeAudioCodecGateway` 通过 `NativeBagBridge` 进入 `jni_bridge.cpp -> bag_api.h`；`mini / flash / pro / ultra` 共用同一组稳定 encode/decode/validate 入口，其中 `flash` 的 emotion preset 会在 config 中透传为 `flash_signal_profile` 与 `flash_voicing_flavor` 两轴，`mini` 通过同一 `frame_samples` 字段承接 Morse speed preset，不再额外分叉 mode-only JNI 方法。
