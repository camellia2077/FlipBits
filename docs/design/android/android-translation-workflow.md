...
- 当 Android 改动涉及 XML 文案、样例文本或本地化结构时，agent / 开发者应该什么时候看翻译文档、什么时候跑 `android-translate` 工作流、失败后去哪里看报告。
...
- 处理构建里和翻译对齐相关的失败
- 想手动使用 `python tools/run.py android-translate ...`
...
- 真正不确定时，优先跑 `key-alignment`，而不是手工目测目录结构。
...
2. `strings-add` 默认只写英文 `values/` 基线，并自动生成 `key-alignment` 报告
...
5. 修改已有 key 时先改英文基线 XML，再用 `android-translate` 生成 review / task / replacement 产物修订对应语言
6. 如果改动涉及样例文本，再检查：
...
- [tools/repo_tooling/android_translate/AGENTS.md](/C:/code/WaveBits/tools/repo_tooling/android_translate/AGENTS.md)
...
> 改文案时先想多语言，构建失败先看 `key-alignment` 报告。
