...
## 情绪模型
`flash` 当前是一次破坏性重构后的六情绪模型：用户可见和代码配置只保留 `Standard / Hostility / Litany / Collapse / Zeal / Void`。每个 emotion 同时决定 payload timing 与 voicing 声效，不再保留旧 flavor 或旧兼容映射。

- `flash_signal_profile`
  - 负责 payload 中每个 bit 的基础时长和 silence slot 尺寸。
  - 当前只包括 `Standard`、`Hostility`、`Litany`、`Collapse`、`Zeal`、`Void`。
  - `Standard` 使用 `0.9375x frame_samples`，carrier 为 `300 / 600 Hz`。
  - `Hostility` 使用 `0.875x frame_samples`，carrier 为更有攻击性的上冲区间 `500-618 / 1000-1236 Hz`。
  ...
  - `Void` 使用 `2.5x / 2.75x / 3x frame_samples` 的缓慢变长 timing，carrier 为下坠区间 `180-230 / 360-460 Hz`，并插入稀疏真实 payload silence。
...
- `Hostility`: `signal=Hostility`, `voicing=Hostility`
...
- `Zeal` 使用确定性变速 / 变频 layout 和专用 gap-aware decode；强标点 silence 仍是真正零样本静音。`Void` 使用确定性下坠 / 变长 layout 和 layout-aware decode；稀疏 silence 表达空白与遗失。
...
| Hostility | Hostility | 连续 `0.875x` bit，`500-618 / 1000-1236 Hz` 上冲 jitter，短 attack / release | 强 click、金属边缘、软削波 | 无额外 silence | 愤怒、敌意、命令 |
...
| Void | Void | `2.5x / 2.75x / 3x` bit，`180-230 / 360-460 Hz` 下坠 sag | 更深低频 drone、极弱 accent、更长 release、下行 shell | 稀疏 `1-6 slot` silence | 悲伤、空虚、残缺 |
