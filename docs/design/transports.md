...
- `ultra` 使用真实 MFSK16：固定 `15.625 Bd`、16 tones、15.625Hz spacing、Gray mapping、Varicode、R=1/2 K=7 FEC 和 10-stage diagonal interleaver；它不再有项目自定义 frame/CRC。
...
| `ultra` | 面向 UTF-8 文本，按 UTF-8 byte 处理 | UTF-8 bytes -> MFSK Varicode -> FEC/interleaver | MFSK16 `15.625 Bd` |
...
- 生成侧 speed preset 为 `10 WPM / 15 WPM / 20 WPM`，属于 Morse 协议速度，不是 style；clean decode 可从 PCM timing 估计实际 unit，不要求依赖 preset metadata。
- 详细设计：
...
- 核心：UTF-8 bytes 进入 MFSK16 Varicode，再经过 FEC/interleaver 后按 Gray nibble 映射到 16 个固定频点。
- 固定速度只有 `15.625 Bd`；decoder 可用 `frame_samples=0`，从采样率和 MFSK16 标准符号率得到符号长度。
- 详细设计：
