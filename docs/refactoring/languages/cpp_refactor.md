...
- 不要为了“全 module”破坏 C 接口、JNI/wasm bridge 或跨平台构建能力；也不要用多个 `.inc` 把同一职责机械切碎
- 若 module source 与不支持 modules 的平台 adapter 必须共享实现，可为一个已命名、内聚的 module/source owner 保留一个 canonical `*_impl.inc`；扫描器会把该实现计入包含它的 owner，不能借此规避职责阈值
