...
void TestApiFlashConfigAffectsLengthAndRoundTrip() {
  ...
  for (const auto& config_case : test::ConfigCases()) {
    const auto standard_encoder = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_STANDARD,
        BAG_FLASH_VOICING_FLAVOR_STANDARD);
    const auto standard_decoder = MakeDecoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_STANDARD,
        BAG_FLASH_VOICING_FLAVOR_STANDARD);
    const auto litany_encoder = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_LITANY,
        BAG_FLASH_VOICING_FLAVOR_LITANY);
    const auto litany_decoder = MakeDecoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_LITANY,
        BAG_FLASH_VOICING_FLAVOR_LITANY);
    const auto hostility_encoder = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_HOSTILITY,
        BAG_FLASH_VOICING_FLAVOR_HOSTILITY);
    const auto hostility_decoder = MakeDecoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_HOSTILITY,
        BAG_FLASH_VOICING_FLAVOR_HOSTILITY);
    const auto zeal_encoder = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_ZEAL,
        BAG_FLASH_VOICING_FLAVOR_ZEAL);
    const auto zeal_decoder = MakeDecoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_ZEAL,
        BAG_FLASH_VOICING_FLAVOR_ZEAL);
    const auto void_encoder = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_VOID,
        BAG_FLASH_VOICING_FLAVOR_VOID);
    const auto void_decoder = MakeDecoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_VOID,
        BAG_FLASH_VOICING_FLAVOR_VOID);
    ...
    test::AssertEq(
        bag_encode_text(&standard_encoder, text.c_str(), &standard_pcm), BAG_OK,
        "standard flash encode should succeed through the C API.");
    test::AssertEq(bag_encode_text(&litany_encoder, text.c_str(), &litany_pcm),
                   BAG_OK,
                   "litany flash encode should succeed through the C API.");
    test::AssertEq(
        bag_encode_text(&hostility_encoder, text.c_str(), &hostility_pcm),
        BAG_OK, "hostility flash encode should succeed through the C API.");
    test::AssertEq(bag_encode_text(&zeal_encoder, text.c_str(), &zeal_pcm),
                   BAG_OK,
                   "zeal flash encode should succeed through the C API.");
    test::AssertEq(bag_encode_text(&void_encoder, text.c_str(), &void_pcm),
                   BAG_OK,
                   "void flash encode should succeed through the C API.");
    test::AssertEq(standard_pcm.sample_count,
                   ExpectedFlashSampleCount(text, config_case,
                                            BAG_FLASH_SIGNAL_PROFILE_STANDARD,
                                            BAG_FLASH_VOICING_FLAVOR_STANDARD),
                   "standard C API flash length should stay on the baseline "
                   "explicit configuration.");
    test::AssertEq(litany_pcm.sample_count,
                   ExpectedFlashSampleCount(text, config_case,
                                            BAG_FLASH_SIGNAL_PROFILE_LITANY,
                                            BAG_FLASH_VOICING_FLAVOR_LITANY),
                   "litany C API flash length should include the longer timing "
                   "and shell configuration.");
    test::AssertTrue(litany_pcm.sample_count > standard_pcm.sample_count,
                     "litany flash output should be longer than standard for "
                     "the same text.");
    test::AssertEq(hostility_pcm.sample_count,
                   ExpectedFlashSampleCount(text, config_case,
                                            BAG_FLASH_SIGNAL_PROFILE_HOSTILITY,
                                            BAG_FLASH_VOICING_FLAVOR_HOSTILITY),
                   "hostility C API flash length should use the faster command "
                   "timing with its own shell.");
    test::AssertTrue(hostility_pcm.sample_count < standard_pcm.sample_count,
                     "hostility flash output should be shorter than standard "
                     "for the same text.");
    test::AssertEq(zeal_pcm.sample_count,
                   ExpectedFlashSampleCount(text, config_case,
                                            BAG_FLASH_SIGNAL_PROFILE_ZEAL,
                                            BAG_FLASH_VOICING_FLAVOR_ZEAL),
                   "zeal C API flash length should use the conservative faster "
                   "signal timing.");
    test::AssertTrue(
    ...
    const auto hostility_decoded =
        DecodeViaApi(hostility_decoder, hostility_pcm);
    const auto zeal_decoded = DecodeViaApi(zeal_decoder, zeal_pcm);
    const auto void_decoded = DecodeViaApi(void_decoder, void_pcm);
    test::AssertEq(standard_decoded.code, BAG_OK,
                   "standard flash decode should succeed.");
    test::AssertEq(litany_decoded.code, BAG_OK,
                   "litany flash decode should succeed.");
    test::AssertEq(hostility_decoded.code, BAG_OK,
                   "hostility flash decode should succeed.");
    test::AssertEq(zeal_decoded.code, BAG_OK,
                   "zeal flash decode should succeed.");
    test::AssertEq(void_decoded.code, BAG_OK,
                   "void flash decode should succeed.");
    test::AssertEq(standard_decoded.text, text,
                   "standard flash decode should preserve text.");
    test::AssertEq(litany_decoded.text, text,
                   "litany flash decode should preserve text.");
    test::AssertEq(hostility_decoded.text, text,
                   "hostility flash decode should preserve text.");
    test::AssertEq(zeal_decoded.text, text,
                   "zeal flash decode should preserve text.");
    test::AssertEq(void_decoded.text, text,
                   "void flash decode should preserve text.");
  ...
  for (const auto& item : cases) {
    const auto encoder =
        MakeEncoderConfig(config_case, BAG_TRANSPORT_FLASH,
                          BAG_FLASH_SIGNAL_PROFILE_STANDARD, item.flavor);
    const auto decoder =
        MakeDecoderConfig(config_case, BAG_TRANSPORT_FLASH,
                          BAG_FLASH_SIGNAL_PROFILE_STANDARD, item.flavor);
    bag_pcm16_result pcm{};
    test::AssertEq(
        bag_validate_encode_request(&encoder, text.c_str()), BAG_VALIDATION_OK,
        std::string(item.name) + " flash voicing flavor should validate.");
    test::AssertEq(
        bag_encode_text(&encoder, text.c_str(), &pcm), BAG_OK,
...
void TestApiFlashVoicingOnlyHostilityKeepsStandardLengthAndCollapseUsesVariableSilence() {
  ...
  const auto hostility_encoder = MakeEncoderConfig(
      config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_STANDARD,
      BAG_FLASH_VOICING_FLAVOR_HOSTILITY);
  const auto collapse_encoder = MakeEncoderConfig(
      config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_STANDARD,
      BAG_FLASH_VOICING_FLAVOR_COLLAPSE);
  const auto collapse_decoder = MakeDecoderConfig(
      config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_STANDARD,
      BAG_FLASH_VOICING_FLAVOR_COLLAPSE);
  ...
  test::AssertEq(
      bag_encode_text(&hostility_encoder, text.c_str(), &hostility_pcm), BAG_OK,
      "hostility flash encode should succeed.");
  test::AssertEq(
      bag_encode_text(&collapse_encoder, text.c_str(), &collapse_pcm), BAG_OK,
      "collapse flash encode should succeed.");
  test::AssertEq(hostility_pcm.sample_count, standard_pcm.sample_count,
                 "hostility voicing alone should keep standard signal timing "
                 "when the signal profile stays standard.");
  test::AssertTrue(collapse_pcm.sample_count > standard_pcm.sample_count,
                   "collapse should include variable hesitation silence in its "
                   "payload length.");
  const auto collapse_decoded = DecodeViaApi(collapse_decoder, collapse_pcm);
  test::AssertEq(collapse_decoded.code, BAG_OK,
                 "collapse flash decode should skip variable silence.");
  test::AssertEq(collapse_decoded.text, text,
                 "collapse variable silence should preserve text.");
  bag_free_pcm16_result(&standard_pcm);
  bag_free_pcm16_result(&hostility_pcm);
...
void TestApiFlashDecodeRequiresMatchingConfig() {
  const auto config_case = test::ConfigCases().front();
  const auto ritual_encoder = MakeEncoderConfig(
      config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_LITANY,
      BAG_FLASH_VOICING_FLAVOR_LITANY);
  const auto wrong_decoder = MakeDecoderConfig(
      config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_STANDARD,
      BAG_FLASH_VOICING_FLAVOR_STANDARD);
  const std::string text = "Mismatch";

  bag_pcm16_result ritual_pcm{};
  test::AssertEq(bag_encode_text(&ritual_encoder, text.c_str(), &ritual_pcm),
                 BAG_OK,
                 "litany flash encode should succeed before wrong-style decode "
                 "validation.");

  const auto decoded = DecodeViaApi(wrong_decoder, ritual_pcm);
  test::AssertTrue(decoded.code != BAG_OK || decoded.text != text,
                   "Decoding litany flash PCM with standard signal/flavor "
                   "should not look like a valid roundtrip.");
  bag_free_pcm16_result(&ritual_pcm);

  const auto zeal_encoder = MakeEncoderConfig(config_case, BAG_TRANSPORT_FLASH,
                                              BAG_FLASH_SIGNAL_PROFILE_ZEAL,
                                              BAG_FLASH_VOICING_FLAVOR_ZEAL);
  const std::string zeal_text = "ZealMismatch";
  bag_pcm16_result zeal_pcm{};
  test::AssertEq(
      bag_encode_text(&zeal_encoder, zeal_text.c_str(), &zeal_pcm), BAG_OK,
  ...
  test::AssertTrue(
      ...
      "Decoding zeal flash PCM with standard signal/flavor should not look "
      "like a valid roundtrip.");
  bag_free_pcm16_result(&zeal_pcm);

  const auto void_encoder = MakeEncoderConfig(config_case, BAG_TRANSPORT_FLASH,
                                              BAG_FLASH_SIGNAL_PROFILE_VOID,
                                              BAG_FLASH_VOICING_FLAVOR_VOID);
  const std::string void_text = "VoidMismatch";
  bag_pcm16_result void_pcm{};
  test::AssertEq(
      bag_encode_text(&void_encoder, void_text.c_str(), &void_pcm), BAG_OK,
  ...
  test::AssertTrue(
      ...
      "Decoding void flash PCM with standard signal/flavor should not look "
      "like a valid roundtrip.");
  bag_free_pcm16_result(&void_pcm);
}
...
void TestApiFlashZealVariableCadenceRoundTrip() {
  ...
  const auto zeal_encoder = MakeEncoderConfig(config_case, BAG_TRANSPORT_FLASH,
                                              BAG_FLASH_SIGNAL_PROFILE_ZEAL,
                                              BAG_FLASH_VOICING_FLAVOR_ZEAL);
  const auto zeal_decoder = MakeDecoderConfig(config_case, BAG_TRANSPORT_FLASH,
                                              BAG_FLASH_SIGNAL_PROFILE_ZEAL,
                                              BAG_FLASH_VOICING_FLAVOR_ZEAL);
  const std::string text = "Zeal!Burst";

  bag_pcm16_result pcm{};
  test::AssertEq(bag_encode_text(&zeal_encoder, text.c_str(), &pcm), BAG_OK,
                 "zeal variable-cadence flash encode should succeed.");
  test::AssertEq(
      pcm.sample_count,
      ExpectedFlashSampleCount(text, config_case, BAG_FLASH_SIGNAL_PROFILE_ZEAL,
                               BAG_FLASH_VOICING_FLAVOR_ZEAL),
      "zeal variable-cadence flash length should include deterministic "
      "punctuation pauses.");

  const auto decoded = DecodeViaApi(zeal_decoder, pcm);
  test::AssertEq(decoded.code, BAG_OK,
                 "zeal variable-cadence decode should succeed.");
  test::AssertEq(decoded.text, text,
                 "zeal variable-cadence decode should preserve text.");
  bag_free_pcm16_result(&pcm);
}
...
void TestApiFlashSignalInfoDerivedFromLayout() {
  ...
  test::AssertTrue(standard_info.available != 0,
                   "standard flash signal info should be available.");
  test::AssertEq(
      std::string(low_buffer.data(), standard_info.low_carrier_hz_size), "300",
      "standard low carrier info should match the rendered layout.");
  test::AssertEq(
      std::string(high_buffer.data(), standard_info.high_carrier_hz_size),
      "600", "standard high carrier info should match the rendered layout.");
  test::AssertEq(
      std::string(bit_buffer.data(), standard_info.bit_duration_samples_size),
      std::to_string((static_cast<std::size_t>(config_case.frame_samples) *
                      static_cast<std::size_t>(15)) /
                     static_cast<std::size_t>(16)),
      "standard bit duration info should match the conservative faster "
      "timing.");
  test::AssertEq(
      std::string(silence_buffer.data(), standard_info.payload_silence_size),
      "none", "standard silence info should report no payload silence.");
  test::AssertEq(
      std::string(decode_buffer.data(), standard_info.decode_path_size),
  ...
  test::AssertEq(bag_describe_flash_signal(&zeal_encoder, "A!B", &zeal_info),
                 BAG_OK,
                 "zeal flash signal info should include variable layout data.");
  test::AssertEq(
      std::string(low_buffer.data(), zeal_info.low_carrier_hz_size),
      "560 / 660 / 760 / 900",
      "zeal low carrier info should expose all rendered low carriers.");
  test::AssertEq(
      std::string(high_buffer.data(), zeal_info.high_carrier_hz_size),
      "1120 / 1320 / 1520 / 1800",
      "zeal high carrier info should expose all rendered high carriers.");
  test::AssertEq(
      std::string(bit_buffer.data(), zeal_info.bit_duration_samples_size),
      "1102 / 1378 / 1653 / 2205",
      "zeal bit duration info should expose variable sample windows.");
  test::AssertEq(
      std::string(silence_buffer.data(), zeal_info.payload_silence_size),
      "4 slot gap",
  ...
  test::AssertEq(
      bag_describe_flash_signal(&void_encoder, "InfoLoss", &void_info), BAG_OK,
      "void flash signal info should be derived from the core layout.");
  test::AssertEq(
      std::string(low_buffer.data(), void_info.low_carrier_hz_size),
      "180 / 184 / 190 / 194 / 198 / 206 / 208 / 214 / 222 / 230",
      "void low carrier info should expose the downward carrier phrase.");
  test::AssertEq(
      std::string(high_buffer.data(), void_info.high_carrier_hz_size),
      "360 / 368 / 380 / 388 / 396 / 412 / 416 / 428 / 444 / 460",
      "void high carrier info should expose the doubled downward carrier "
      "phrase.");
  test::AssertEq(
      std::string(bit_buffer.data(), void_info.bit_duration_samples_size),
      "5512 / 6063 / 6615",
      "void bit duration info should expose baseline, tail, and loss-bit "
      "timing.");
  test::AssertEq(
      std::string(silence_buffer.data(), void_info.payload_silence_size),
      "1 / 2 / 4 slot gap",
...
void RegisterApiFlashTests(test::Runner& runner) {
  runner.Add("Api.FlashConfigAffectsLengthAndRoundTrip",
             TestApiFlashConfigAffectsLengthAndRoundTrip);
  runner.Add("Api.FlashVoicingEmotionValuesRoundTrip",
             TestApiFlashVoicingEmotionValuesRoundTrip);
  runner.Add(
      "Api."
      "FlashVoicingOnlyHostilityKeepsStandardLengthAndCollapseUsesVariableSilen"
      "ce",
      TestApiFlashVoicingOnlyHostilityKeepsStandardLengthAndCollapseUsesVariableSilence);
  runner.Add("Api.FlashDecodeRequiresMatchingConfig",
             TestApiFlashDecodeRequiresMatchingConfig);
  runner.Add("Api.FlashZealVariableCadenceRoundTrip",
             TestApiFlashZealVariableCadenceRoundTrip);
  runner.Add("Api.FlashSignalInfoDerivedFromLayout",
             TestApiFlashSignalInfoDerivedFromLayout);
}
