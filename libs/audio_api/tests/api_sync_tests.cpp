...
std::size_t MorseToneElementCountForText(const std::string& value) {
  auto pattern_size = [](const char raw) -> std::size_t {
    const char ch =
        raw >= 'a' && raw <= 'z' ? static_cast<char>('A' + (raw - 'a')) : raw;
    switch (ch) {
      case 'A':
        return 2;
      case 'B':
  ...
  for (const auto& config_case : test::ConfigCases()) {
    ...
    for (const auto& item : cases) {
      const std::string context =
          "input=" + test::detail::ToDebugString(item.input);
      bag_pcm16_result pcm{};
      test::AssertEq(bag_encode_text(&encoder_config, item.input.c_str(), &pcm),
                     BAG_OK,
      ...
      test::AssertEqWithContext(
          decoded.text, item.expected_text,
          "Mini Morse roundtrip should preserve normalized text.", context);
      test::AssertEq(decoded.mode, BAG_TRANSPORT_MINI,
                     "Mini Morse decode should preserve mode.");
...
void TestApiEncodeRejectsInvalidArguments() {
  ...
  test::AssertEq(bag_encode_text(nullptr, "A", &pcm), BAG_INVALID_ARGUMENT,
                 "Null encoder config should be rejected.");
  test::AssertEq(bag_encode_text(&encoder_config, nullptr, &pcm),
                 BAG_INVALID_ARGUMENT, "Null text should be rejected.");
  test::AssertEq(bag_encode_text(&encoder_config, "A", nullptr),
  ...
  test::AssertEq(bag_encode_text(&invalid_config, "A", &pcm),
                 BAG_INVALID_ARGUMENT, "Zero sample rate should be rejected.");

  invalid_config = encoder_config;
  invalid_config.frame_samples = 0;
  test::AssertEq(bag_encode_text(&invalid_config, "A", &pcm),
                 BAG_INVALID_ARGUMENT, "Zero frame size should be rejected.");
...
void TestApiCreateDecoderRejectsInvalidArguments() {
  ...
  test::AssertEq(bag_create_decoder(nullptr, &decoder), BAG_INVALID_ARGUMENT,
                 "Null decoder config should be rejected.");
  test::AssertEq(bag_create_decoder(&decoder_config, nullptr),
                 BAG_INVALID_ARGUMENT,
...
void TestApiPollAndResetLifecycle() {
  ...
  bag_pcm16_result pcm{};
  test::AssertEq(bag_encode_text(&encoder_config, "RESET", &pcm), BAG_OK,
...
void TestApiModeSpecificValidation() {
  ...
  test::AssertEq(bag_encode_text(&pro_config, pro_non_ascii.c_str(), &pcm),
                 BAG_INVALID_ARGUMENT,
  ...
  test::AssertEq(
      bag_encode_text(&ultra_config, test::BuildTooLongUltraCorpus().c_str(),
                      &pcm),
      BAG_OK,
  ...
  test::AssertEq(bag_encode_text(&mini_config, mini_non_morse.c_str(), &pcm),
                 BAG_INVALID_ARGUMENT,
                 "Mini Morse mode should reject non-Morse text.");
  test::AssertEq(
      bag_encode_text(&mini_config, "PRAISE THE OMNISSIAH", &pcm), BAG_OK,
      "Mini Morse mode should accept the pro-style ASCII example text.");
  test::AssertEq(bag_encode_text(&mini_config, "", &pcm), BAG_INVALID_ARGUMENT,
                 "Mini Morse mode should reject a truly empty string while "
                 "still allowing encodable whitespace.");
  bag_free_pcm16_result(&pcm);
}
...
void TestApiBoundarySuccessCases() {
...

  {
    ...
    bag_pcm16_result pcm{};
    test::AssertEq(
        bag_encode_text(&encoder_config, long_flash_text.c_str(), &pcm), BAG_OK,
        "Flash mode should not inherit the single-frame payload limit.");
    const auto decoded = DecodeViaApi(decoder_config, pcm);
    test::AssertEq(decoded.code, BAG_OK,
                   "Flash long-text decode should succeed.");
    test::AssertEq(decoded.text, long_flash_text,
                   "Flash long-text roundtrip should preserve text.");
    test::AssertEq(decoded.mode, BAG_TRANSPORT_FLASH,
                   "Flash long-text decode should preserve mode.");
    bag_free_pcm16_result(&pcm);
  }

  {
    const auto encoder_config =
        MakeEncoderConfig(config_case, BAG_TRANSPORT_PRO);
    const auto decoder_config =
        MakeDecoderConfig(config_case, BAG_TRANSPORT_PRO);
    bag_pcm16_result pcm{};
    test::AssertEq(bag_encode_text(&encoder_config,
                                   test::BuildTooLongProCorpus().c_str(), &pcm),
                   BAG_OK,
                   "Pro mode should accept extended ASCII text beyond the old "
                   "compat limit.");
    const auto decoded = DecodeViaApi(decoder_config, pcm);
    test::AssertEq(decoded.code, BAG_OK, "Pro boundary decode should succeed.");
    test::AssertEq(
        decoded.text, test::BuildTooLongProCorpus(),
        "Pro boundary roundtrip should preserve the extended ASCII text.");
    test::AssertEq(decoded.mode, BAG_TRANSPORT_PRO,
                   "Pro boundary decode should preserve mode.");
    bag_free_pcm16_result(&pcm);
  }

  {
    ...
    const auto decoder_config =
        MakeDecoderConfig(config_case, BAG_TRANSPORT_ULTRA);
    bag_pcm16_result pcm{};
    test::AssertEq(
        bag_encode_text(&encoder_config,
                        test::BuildTooLongUltraCorpus().c_str(), &pcm),
        BAG_OK,
        "Ultra mode should accept extended UTF-8 text beyond the old compat "
        "limit.");
    const auto decoded = DecodeViaApi(decoder_config, pcm);
    test::AssertEq(decoded.code, BAG_OK,
                   "Ultra boundary decode should succeed.");
    test::AssertEq(
        decoded.text, test::BuildTooLongUltraCorpus(),
        "Ultra boundary roundtrip should preserve the extended UTF-8 text.");
    test::AssertEq(decoded.mode, BAG_TRANSPORT_ULTRA,
                   "Ultra boundary decode should preserve mode.");
    bag_free_pcm16_result(&pcm);
  }
}
...
void TestApiChunkedPushRoundTripAcrossModes() {
  ...
  for (const auto& config_case : test::ConfigCases()) {
    for (const auto& item : cases) {
      ...
      bag_pcm16_result pcm{};
      test::AssertEq(bag_encode_text(&encoder_config, item.text.c_str(), &pcm),
                     BAG_OK, "Chunked decode setup encode should succeed.");

      const std::size_t chunk_size = std::max<std::size_t>(
          1, pcm.sample_count / static_cast<std::size_t>(7));
      const auto decoded =
          DecodeViaApiInChunks(decoder_config, pcm, chunk_size);
      test::AssertEq(decoded.code, BAG_OK,
                     "Chunked API decode should succeed.");
      test::AssertEq(decoded.text, item.text,
                     "Chunked API decode should preserve text.");
      test::AssertEq(decoded.mode, item.mode,
                     "Chunked API decode should preserve mode.");
      bag_free_pcm16_result(&pcm);
    }
  }
}
...
void TestApiDecodeResultPublishesRawPayloadAcrossModes() {
  ...
  const std::array<RawCase, 9> cases = {{
      ...
      {BAG_TRANSPORT_FLASH, test::Utf8Literal(u8"Flash深仪"),
       BAG_FLASH_SIGNAL_PROFILE_LITANY, BAG_FLASH_VOICING_FLAVOR_LITANY},
      {BAG_TRANSPORT_FLASH, "MidRitual", BAG_FLASH_SIGNAL_PROFILE_LITANY,
       BAG_FLASH_VOICING_FLAVOR_LITANY},
      {BAG_TRANSPORT_FLASH, "ZealRaw", BAG_FLASH_SIGNAL_PROFILE_ZEAL,
       BAG_FLASH_VOICING_FLAVOR_ZEAL},
      {BAG_TRANSPORT_FLASH, "VoidRaw", BAG_FLASH_SIGNAL_PROFILE_VOID,
  ...
  for (const auto& config_case : test::ConfigCases()) {
    for (const auto& item : cases) {
      const auto encoder_config = MakeEncoderConfig(
          config_case, item.mode, item.signal_profile, item.voicing_flavor);
      const auto decoder_config = MakeDecoderConfig(
          config_case, item.mode, item.signal_profile, item.voicing_flavor);
      bag_pcm16_result pcm{};
      test::AssertEq(bag_encode_text(&encoder_config, item.text.c_str(), &pcm),
                     BAG_OK, "Raw-payload setup encode should succeed.");
      const auto decoded = DecodeViaApi(decoder_config, pcm);
      test::AssertEq(decoded.code, BAG_OK,
                     "Raw-payload decode should succeed.");
      test::AssertTrue(decoded.raw_payload_available,
                       "Raw payload should be available after decode.");
      test::AssertEq(decoded.text_status, BAG_DECODE_CONTENT_STATUS_OK,
                     "Structured decode should keep successful text status.");
      test::AssertTrue(
          !decoded.raw_bytes_hex.empty() || item.text.empty(),
          "Raw bytes hex should be populated for non-empty payloads.");
      test::AssertTrue(
          !decoded.raw_bits_binary.empty() || item.text.empty(),
          "Raw bits binary should be populated for non-empty payloads.");
      test::AssertEq(decoded.mode, item.mode,
                     "Raw-payload decode should preserve mode.");
      bag_free_pcm16_result(&pcm);
    }
  }
}
...
void TestApiDecodeResultBufferBoundaries() {
  ...
  const auto decoder_config =
      MakeDecoderConfig(config_case, BAG_TRANSPORT_FLASH);
  bag_pcm16_result pcm{};
  test::AssertEq(bag_encode_text(&encoder_config, "RAW", &pcm), BAG_OK,
...
void TestApiStructuredEncodePublishesFollowAcrossModes() {
  ...
  const std::array<FollowCase, 9> cases = {{
      ...
      {BAG_TRANSPORT_FLASH, "Deep", BAG_FLASH_SIGNAL_PROFILE_LITANY,
       BAG_FLASH_VOICING_FLAVOR_LITANY},
      {BAG_TRANSPORT_FLASH, "Collapse", BAG_FLASH_SIGNAL_PROFILE_COLLAPSE,
       BAG_FLASH_VOICING_FLAVOR_COLLAPSE},
      {BAG_TRANSPORT_FLASH, "Zeal", BAG_FLASH_SIGNAL_PROFILE_ZEAL,
       BAG_FLASH_VOICING_FLAVOR_ZEAL},
      {BAG_TRANSPORT_FLASH, "Void", BAG_FLASH_SIGNAL_PROFILE_VOID,
       BAG_FLASH_VOICING_FLAVOR_VOID},
  ...
  for (const auto& config_case : test::ConfigCases()) {
    for (const auto& item : cases) {
      ...
      result.follow_data.ultra_frame_timeline_buffer =
          ultra_frame_entries.data();
      result.follow_data.ultra_frame_timeline_buffer_count =
          ultra_frame_entries.size();

      test::AssertEq(bag_encode_text_with_follow(&encoder_config,
                                                 item.text.c_str(), &result),
                     BAG_OK, "Structured encode should succeed.");
      test::AssertTrue(result.raw_payload_available != 0,
                       "Structured encode should publish raw payload.");
      test::AssertTrue(result.follow_data.available != 0,
                       "Structured encode should publish follow data.");
      test::AssertTrue(result.sample_count > 0,
                       "Structured encode should return PCM samples.");

      const auto hex_tokens = SplitSpaceSeparatedTokens(
          std::string(raw_hex_buffer.data(), result.raw_bytes_hex_size));
      ...
      test::AssertEq(result.follow_data.binary_group_timeline_count,
                     expected_binary_group_count,
                     "Binary group timeline count should match the "
      ...
      if (item.mode == BAG_TRANSPORT_MINI) {
        ...
        for (std::size_t index = 0;
          ...
          test::AssertEq(entry.bit_count, static_cast<std::size_t>(1),
                         "Mini Morse follow groups should advance one source "
                         "element at a time.");
          saw_dash_duration =
              saw_dash_duration ||
              entry.sample_count >=
        ...
        test::AssertTrue(saw_dash_duration,
                         "Mini Morse follow should keep dash length in "
      ...
      if (item.mode == BAG_TRANSPORT_ULTRA) {
        ...
        if (result.follow_data.byte_timeline_count > 0) {
          test::AssertEq(byte_entries[0].start_sample,
                         ...
                         "Ultra first payload byte should begin at the payload "
                         "start sample.");
        }
        for (std::size_t index = 0;
             index < result.follow_data.binary_group_timeline_count; ++index) {
...
void TestApiEncodeAndDecodeFollowStayAligned() {
  ...
  for (const auto& config_case : test::ConfigCases()) {
    for (const auto mode : modes) {
      ...
      std::array<bag_payload_follow_binary_group_entry, 4096>
          encode_binary_entries{};
      std::array<bag_ultra_frame_symbol_entry, 4096>
          encode_ultra_frame_entries{};
      ...
      encode_result.follow_data.byte_timeline_buffer =
          encode_byte_entries.data();
      encode_result.follow_data.byte_timeline_buffer_count =
          encode_byte_entries.size();
      encode_result.follow_data.binary_group_timeline_buffer =
          encode_binary_entries.data();
      encode_result.follow_data.binary_group_timeline_buffer_count =
          encode_binary_entries.size();
      encode_result.follow_data.ultra_frame_timeline_buffer =
          encode_ultra_frame_entries.data();
      encode_result.follow_data.ultra_frame_timeline_buffer_count =
          encode_ultra_frame_entries.size();
      test::AssertEq(
          bag_encode_text_with_follow(&encoder_config, "FOLLOW",
                                      &encode_result),
          BAG_OK,
      ...
      test::AssertEq(bag_push_pcm(decoder, encode_result.samples,
                                  encode_result.sample_count, 0),
                     BAG_OK,
                     "PCM push should succeed before structured decode follow "
                     "comparison.");
      ...
      std::array<bag_payload_follow_binary_group_entry, 4096>
          decode_binary_entries{};
      std::array<bag_ultra_frame_symbol_entry, 4096>
          decode_ultra_frame_entries{};
      ...
      decode_result.follow_data.byte_timeline_buffer =
          decode_byte_entries.data();
      decode_result.follow_data.byte_timeline_buffer_count =
          decode_byte_entries.size();
      decode_result.follow_data.binary_group_timeline_buffer =
          decode_binary_entries.data();
      decode_result.follow_data.binary_group_timeline_buffer_count =
          decode_binary_entries.size();
      decode_result.follow_data.ultra_frame_timeline_buffer =
          decode_ultra_frame_entries.data();
      decode_result.follow_data.ultra_frame_timeline_buffer_count =
          decode_ultra_frame_entries.size();
      test::AssertEq(bag_poll_decode_result(decoder, &decode_result), BAG_OK,
                     "Structured decode should succeed for follow comparison.");
      bag_destroy_decoder(decoder);

      test::AssertEq(std::string(encode_raw_hex_buffer.data(),
                                 encode_result.raw_bytes_hex_size),
                     std::string(decode_raw_hex_buffer.data(),
                                 decode_result.raw_bytes_hex_size),
      ...
      test::AssertEq(encode_result.follow_data.ultra_frame_timeline_count,
                     decode_result.follow_data.ultra_frame_timeline_count,
                     "Encode-side and decode-side Ultra frame timeline counts "
                     "should match.");
      if (mode == BAG_TRANSPORT_ULTRA) {
        ...
        test::AssertEq(decode_ultra_frame_entries[0].section,
                       BAG_ULTRA_FRAME_SECTION_PREAMBLE,
                       "Structured Ultra decode frame timeline should start "
                       "with preamble.");
      }
...
void TestApiBuildEncodeFollowDataMatchesStructuredEncode() {
  ...
  for (const auto& config_case : test::ConfigCases()) {
    for (const auto mode : modes) {
      ...
      std::array<bag_payload_follow_binary_group_entry, 4096>
          structured_binary_entries{};
      std::array<bag_ultra_frame_symbol_entry, 4096>
          structured_ultra_frame_entries{};
      std::array<bag_text_follow_token_entry, 64> structured_text_entries{};
      std::array<bag_text_follow_raw_segment_entry, 64>
          structured_text_raw_segments{};
      std::array<bag_text_follow_raw_display_unit_entry, 256>
          structured_text_raw_display_units{};
      std::array<bag_text_follow_lyric_line_entry, 64>
          structured_lyric_line_entries{};
      std::array<bag_text_follow_line_token_range_entry, 64>
          structured_line_token_ranges{};
      std::array<bag_text_follow_line_raw_segment_entry, 64>
          structured_line_raw_segments{};
      bag_encode_result structured_result{};
      structured_result.raw_bytes_hex_buffer = structured_raw_hex_buffer.data();
      structured_result.raw_bytes_hex_buffer_size =
          structured_raw_hex_buffer.size();
      structured_result.raw_bits_binary_buffer =
          structured_raw_bits_buffer.data();
      structured_result.raw_bits_binary_buffer_size =
          structured_raw_bits_buffer.size();
      structured_result.text_follow_data.text_tokens_buffer =
          structured_text_tokens_buffer.data();
      structured_result.text_follow_data.text_tokens_buffer_size =
          structured_text_tokens_buffer.size();
      structured_result.text_follow_data.lyric_lines_buffer =
          structured_lyric_lines_buffer.data();
      structured_result.text_follow_data.lyric_lines_buffer_size =
          structured_lyric_lines_buffer.size();
      structured_result.text_follow_data.text_token_timeline_buffer =
          structured_text_entries.data();
      structured_result.text_follow_data.text_token_timeline_buffer_count =
          structured_text_entries.size();
      structured_result.text_follow_data.token_raw_segments_buffer =
          structured_text_raw_segments.data();
      structured_result.text_follow_data.token_raw_segments_buffer_count =
          structured_text_raw_segments.size();
      structured_result.text_follow_data.token_raw_display_units_buffer =
          structured_text_raw_display_units.data();
      structured_result.text_follow_data.token_raw_display_units_buffer_count =
          structured_text_raw_display_units.size();
      structured_result.text_follow_data.lyric_line_timeline_buffer =
          structured_lyric_line_entries.data();
      structured_result.text_follow_data.lyric_line_timeline_buffer_count =
          structured_lyric_line_entries.size();
      structured_result.text_follow_data.line_token_ranges_buffer =
          structured_line_token_ranges.data();
      structured_result.text_follow_data.line_token_ranges_buffer_count =
          structured_line_token_ranges.size();
      structured_result.text_follow_data.line_raw_segments_buffer =
          structured_line_raw_segments.data();
      structured_result.text_follow_data.line_raw_segments_buffer_count =
          structured_line_raw_segments.size();
      structured_result.follow_data.byte_timeline_buffer =
          structured_byte_entries.data();
      structured_result.follow_data.byte_timeline_buffer_count =
          structured_byte_entries.size();
      structured_result.follow_data.binary_group_timeline_buffer =
          structured_binary_entries.data();
      structured_result.follow_data.binary_group_timeline_buffer_count =
          structured_binary_entries.size();
      structured_result.follow_data.ultra_frame_timeline_buffer =
          structured_ultra_frame_entries.data();
      structured_result.follow_data.ultra_frame_timeline_buffer_count =
          structured_ultra_frame_entries.size();
      ...
      std::array<bag_payload_follow_byte_entry, 2048>
          follow_only_byte_entries{};
      std::array<bag_payload_follow_binary_group_entry, 4096>
          follow_only_binary_entries{};
      std::array<bag_ultra_frame_symbol_entry, 4096>
          follow_only_ultra_frame_entries{};
      std::array<bag_text_follow_token_entry, 64> follow_only_text_entries{};
      std::array<bag_text_follow_raw_segment_entry, 64>
          follow_only_text_raw_segments{};
      std::array<bag_text_follow_raw_display_unit_entry, 256>
          follow_only_text_raw_display_units{};
      std::array<bag_text_follow_lyric_line_entry, 64>
          follow_only_lyric_line_entries{};
      std::array<bag_text_follow_line_token_range_entry, 64>
          follow_only_line_token_ranges{};
      std::array<bag_text_follow_line_raw_segment_entry, 64>
          follow_only_line_raw_segments{};
      bag_encode_result follow_only_result{};
      follow_only_result.raw_bytes_hex_buffer =
          follow_only_raw_hex_buffer.data();
      follow_only_result.raw_bytes_hex_buffer_size =
          follow_only_raw_hex_buffer.size();
      follow_only_result.raw_bits_binary_buffer =
          follow_only_raw_bits_buffer.data();
      follow_only_result.raw_bits_binary_buffer_size =
          follow_only_raw_bits_buffer.size();
      follow_only_result.text_follow_data.text_tokens_buffer =
          follow_only_text_tokens_buffer.data();
      follow_only_result.text_follow_data.text_tokens_buffer_size =
          follow_only_text_tokens_buffer.size();
      follow_only_result.text_follow_data.lyric_lines_buffer =
          follow_only_lyric_lines_buffer.data();
      follow_only_result.text_follow_data.lyric_lines_buffer_size =
          follow_only_lyric_lines_buffer.size();
      follow_only_result.text_follow_data.text_token_timeline_buffer =
          follow_only_text_entries.data();
      follow_only_result.text_follow_data.text_token_timeline_buffer_count =
          follow_only_text_entries.size();
      follow_only_result.text_follow_data.token_raw_segments_buffer =
          follow_only_text_raw_segments.data();
      follow_only_result.text_follow_data.token_raw_segments_buffer_count =
          follow_only_text_raw_segments.size();
      follow_only_result.text_follow_data.token_raw_display_units_buffer =
          follow_only_text_raw_display_units.data();
      follow_only_result.text_follow_data.token_raw_display_units_buffer_count =
          follow_only_text_raw_display_units.size();
      follow_only_result.text_follow_data.lyric_line_timeline_buffer =
          follow_only_lyric_line_entries.data();
      follow_only_result.text_follow_data.lyric_line_timeline_buffer_count =
          follow_only_lyric_line_entries.size();
      follow_only_result.text_follow_data.line_token_ranges_buffer =
          follow_only_line_token_ranges.data();
      follow_only_result.text_follow_data.line_token_ranges_buffer_count =
          follow_only_line_token_ranges.size();
      follow_only_result.text_follow_data.line_raw_segments_buffer =
          follow_only_line_raw_segments.data();
      follow_only_result.text_follow_data.line_raw_segments_buffer_count =
          follow_only_line_raw_segments.size();
      follow_only_result.follow_data.byte_timeline_buffer =
          follow_only_byte_entries.data();
      follow_only_result.follow_data.byte_timeline_buffer_count =
          follow_only_byte_entries.size();
      follow_only_result.follow_data.binary_group_timeline_buffer =
          follow_only_binary_entries.data();
      follow_only_result.follow_data.binary_group_timeline_buffer_count =
          follow_only_binary_entries.size();
      follow_only_result.follow_data.ultra_frame_timeline_buffer =
          follow_only_ultra_frame_entries.data();
      follow_only_result.follow_data.ultra_frame_timeline_buffer_count =
          follow_only_ultra_frame_entries.size();

      test::AssertEq(
          bag_encode_text_with_follow(&encoder_config, text.c_str(),
                                      &structured_result),
          BAG_OK,
          "Structured encode should succeed before follow-only comparison.");
      test::AssertEq(bag_build_encode_follow_data(&encoder_config, text.c_str(),
                                                  &follow_only_result),
                     BAG_OK,
                     "Follow-only encode metadata build should succeed.");
      test::AssertEq(
          follow_only_result.sample_count, static_cast<std::size_t>(0),
          "Follow-only metadata build should not allocate PCM output.");
      test::AssertEq(
          std::string(structured_raw_hex_buffer.data(),
                      structured_result.raw_bytes_hex_size),
          std::string(follow_only_raw_hex_buffer.data(),
                      follow_only_result.raw_bytes_hex_size),
          "Follow-only metadata build should match structured encode raw hex.");
      test::AssertEq(std::string(structured_raw_bits_buffer.data(),
                                 structured_result.raw_bits_binary_size),
                     std::string(follow_only_raw_bits_buffer.data(),
                                 follow_only_result.raw_bits_binary_size),
                     "Follow-only metadata build should match structured "
                     "encode raw bits.");
      test::AssertEq(
      ...
      test::AssertEq(structured_result.follow_data.ultra_frame_timeline_count,
                     follow_only_result.follow_data.ultra_frame_timeline_count,
                     "Follow-only metadata build should preserve Ultra frame "
                     "timeline count.");
      test::AssertEq(
          ...
          "Follow-only metadata build should preserve text token timeline "
          "count.");
      test::AssertEq(structured_result.text_follow_data.text_characters_count,
                     follow_only_result.text_follow_data.text_characters_count,
                     "Follow-only metadata build should preserve character "
                     "timeline count.");
      test::AssertEq(
...
void TestApiFlashFollowTimingRespectsStyleRules() {
  ...
  const std::array<StyleCase, 5> cases = {{
      {BAG_FLASH_SIGNAL_PROFILE_STANDARD, BAG_FLASH_VOICING_FLAVOR_STANDARD, 15,
       16, 3, 0.0},
      {BAG_FLASH_SIGNAL_PROFILE_LITANY, BAG_FLASH_VOICING_FLAVOR_LITANY, 6, 1,
       0, 1.35},
      {BAG_FLASH_SIGNAL_PROFILE_HOSTILITY, BAG_FLASH_VOICING_FLAVOR_HOSTILITY,
       7, 8, 3, 0.0},
      {BAG_FLASH_SIGNAL_PROFILE_COLLAPSE, BAG_FLASH_VOICING_FLAVOR_COLLAPSE, 1,
       1, 3, 0.0},
      {BAG_FLASH_SIGNAL_PROFILE_ZEAL, BAG_FLASH_VOICING_FLAVOR_ZEAL, 1, 2, 3,
       0.0},
  }};
  const auto config_case = test::ConfigCases().front();
  ...
  for (const auto& item : cases) {
    const auto encoder_config =
        MakeEncoderConfig(config_case, BAG_TRANSPORT_FLASH, item.signal_profile,
                          item.voicing_flavor);
    ...
    test::AssertEq(
        bag_encode_text_with_follow(&encoder_config, "AB", &result), BAG_OK,
    ...
    test::AssertEq(result.follow_data.payload_begin_sample,
                   item.expected_payload_begin_frame_multiplier > 0
                       ...
                       : static_cast<std::size_t>(std::lround(
                             static_cast<double>(config_case.sample_rate_hz) *
                             item.expected_payload_begin_seconds)),
                   "Flash follow payload begin should stay style-aware.");
    test::AssertTrue(result.follow_data.binary_group_timeline_count > 0,
  ...
  {
    const auto encoder_config = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_ZEAL,
        BAG_FLASH_VOICING_FLAVOR_ZEAL);
    std::array<char, 1024> raw_hex_buffer{};
    std::array<char, 8192> raw_bits_buffer{};
    std::array<bag_payload_follow_byte_entry, 512> byte_entries{};
    ...
    test::AssertEq(bag_encode_text_with_follow(&encoder_config, "A!B", &result),
                   BAG_OK,
                   "Zeal follow encode should support variable timing and "
                   "punctuation pauses.");
    test::AssertEq(result.follow_data.binary_group_timeline_count,
                   static_cast<std::size_t>(24),
                   "Zeal follow data should still publish one binary group per "
                   "payload bit.");
    test::AssertEq(binary_entries[0].sample_count,
                   static_cast<std::size_t>(config_case.frame_samples) /
                       static_cast<std::size_t>(2),
                   "Zeal first bit should use burst timing.");
    test::AssertEq(
        binary_entries[0].carrier_freq_hz, 900.0,
        "Zeal first low bit should publish the rendered burst carrier.");
    test::AssertEq(binary_entries[2].sample_count,
                   static_cast<std::size_t>(config_case.frame_samples) *
                       static_cast<std::size_t>(5) /
                       static_cast<std::size_t>(8),
                   "Zeal should vary bit duration inside the deterministic "
                   "burst pattern.");
    test::AssertEq(
        binary_entries[2].carrier_freq_hz, 760.0,
    ...
    test::AssertEq(gap_after_exclamation,
                   static_cast<std::size_t>(config_case.frame_samples) *
                       static_cast<std::size_t>(4),
                   "Zeal strong punctuation should add a long payload pause "
                   "before the next burst.");
    test::AssertEq(binary_entries[16].sample_count,
                   static_cast<std::size_t>(config_case.frame_samples) /
                       static_cast<std::size_t>(2),
                   "Zeal should restart with a burst bit immediately after a "
                   "punctuation pause.");
    test::AssertEq(
        binary_entries[16].carrier_freq_hz, 900.0,
  ...
  {
    const auto encoder_config = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_VOID,
        BAG_FLASH_VOICING_FLAVOR_VOID);
    std::array<char, 1024> raw_hex_buffer{};
    std::array<char, 8192> raw_bits_buffer{};
    std::array<bag_payload_follow_byte_entry, 512> byte_entries{};
    ...
    test::AssertEq(bag_encode_text_with_follow(&encoder_config, "AB", &result),
                   BAG_OK,
                   "Void follow encode should support variable sag timing and "
                   "sparse gaps.");
    test::AssertTrue(
        result.follow_data.binary_group_timeline_count >=
            static_cast<std::size_t>(8),
    ...
    test::AssertEq(binary_entries[7].sample_count,
                   static_cast<std::size_t>(config_case.frame_samples) *
                       static_cast<std::size_t>(11) /
                       static_cast<std::size_t>(4),
                   "Void byte-tail bit should stretch slightly longer.");
    test::AssertEq(binary_entries[0].carrier_freq_hz, 230.0,
                   "Void first low bit should publish the start of the "
                   "downward carrier phrase.");
    test::AssertEq(
        binary_entries[1].carrier_freq_hz, 444.0,
  ...
  {
    const auto encoder_config = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_HOSTILITY,
    ...
    test::AssertEq(bag_encode_text_with_follow(&encoder_config, "A", &result),
                   BAG_OK,
                   "Hostility follow encode should support deterministic "
    ...
    test::AssertEq(
        binary_entries[0].carrier_freq_hz, 500.0,
        "Hostility first low bit should publish its upward jittered carrier.");
    test::AssertEq(binary_entries[1].carrier_freq_hz, 1140.0,
                   "Hostility second high bit should publish the doubled "
                   "upward jittered carrier.");
  ...
  {
    const auto encoder_config = MakeEncoderConfig(
        config_case, BAG_TRANSPORT_FLASH, BAG_FLASH_SIGNAL_PROFILE_COLLAPSE,
        BAG_FLASH_VOICING_FLAVOR_COLLAPSE);
    std::array<char, 1024> raw_hex_buffer{};
    std::array<char, 8192> raw_bits_buffer{};
    std::array<bag_payload_follow_byte_entry, 512> byte_entries{};
    ...
    test::AssertEq(bag_encode_text_with_follow(&encoder_config, "A", &result),
                   BAG_OK,
                   "Collapse follow encode should support deterministic "
                   "per-bit carrier sag.");
    test::AssertTrue(
        result.follow_data.binary_group_timeline_count >=
            static_cast<std::size_t>(2),
        "Collapse follow data should publish the first two binary bits.");
    test::AssertEq(binary_entries[0].carrier_freq_hz, 320.0,
                   "Collapse first low bit should publish the sag carrier.");
    test::AssertEq(
        binary_entries[1].carrier_freq_hz, 610.0,
...
void TestApiStructuredEncodePublishesWordLevelTextFollow() {
  ...
  std::array<bag_text_follow_raw_display_unit_entry, 64>
  ...
  test::AssertEq(
      bag_encode_text_with_follow(&encoder_config, "ASH BELL", &result), BAG_OK,
      "Structured encode should succeed for word-level text follow.");
  test::AssertTrue(result.text_follow_data.available != 0,
                   "Encode should publish text follow data.");
  test::AssertEq(result.text_follow_data.text_tokens_status,
                 BAG_DECODE_CONTENT_STATUS_OK,
                 "Encode should publish text-follow tokens.");
  test::AssertEq(result.text_follow_data.text_token_timeline_status,
                 BAG_DECODE_CONTENT_STATUS_OK,
                 "Encode should publish text-follow timeline entries.");
  test::AssertEq(result.text_follow_data.token_raw_segments_status,
                 BAG_DECODE_CONTENT_STATUS_OK,
                 "Encode should publish token raw-segment mappings.");
  test::AssertEq(result.text_follow_data.token_raw_display_units_status,
                 BAG_DECODE_CONTENT_STATUS_OK,
                 "Encode should publish token raw display units.");
  test::AssertEq(result.text_follow_data.lyric_lines_status,
                 BAG_DECODE_CONTENT_STATUS_OK,
                 "Encode should publish lyric lines.");
  test::AssertEq(result.text_follow_data.lyric_line_timeline_status,
                 BAG_DECODE_CONTENT_STATUS_OK,
                 "Encode should publish lyric line timeline entries.");
  test::AssertEq(result.text_follow_data.line_token_ranges_status,
                 BAG_DECODE_CONTENT_STATUS_OK,
                 "Encode should publish line token ranges.");
  test::AssertEq(result.text_follow_data.line_raw_segments_status,
                 BAG_DECODE_CONTENT_STATUS_OK,
                 "Encode should publish line raw-segment mappings.");
  const auto tokens = SplitLineSeparatedTokens(std::string(
      text_tokens_buffer.data(), result.text_follow_data.text_tokens_size));
  const auto lyric_lines = SplitLineSeparatedTokens(std::string(
      lyric_lines_buffer.data(), result.text_follow_data.lyric_lines_size));
  test::AssertEq(tokens.size(), static_cast<std::size_t>(2),
                 "Space-delimited text should tokenize by word.");
  test::AssertEq(tokens[0], std::string("ASH"),
                 "First token should preserve the first word.");
  test::AssertEq(tokens[1], std::string("BELL"),
                 "Second token should preserve the second word.");
  test::AssertEq(
      result.text_follow_data.text_token_timeline_count, tokens.size(),
      "Word-level text follow should align one timeline entry per token.");
  test::AssertEq(text_entries.front().start_sample,
                 result.follow_data.payload_begin_sample,
  ...
  test::AssertEq(
      result.text_follow_data.token_raw_segments_count, tokens.size(),
      "Word-level text follow should publish one raw segment per token.");
  test::AssertEq(result.text_follow_data.lyric_line_timeline_count,
                 lyric_lines.size(),
                 "Lyric lines should align one timeline entry per line.");
  test::AssertEq(
      lyric_lines.size(), static_cast<std::size_t>(1),
      "Short space-delimited text should stay on a single lyric line.");
  test::AssertEq(lyric_lines.front(), std::string("ASH BELL"),
                 "Single lyric line should preserve word spacing.");
  test::AssertEq(result.text_follow_data.line_token_ranges_count,
                 lyric_lines.size(),
                 "Lyric lines should publish one token-range entry per line.");
  test::AssertEq(result.text_follow_data.line_raw_segments_count,
                 lyric_lines.size(),
                 "Lyric lines should publish one raw-segment entry per line.");
  test::AssertEq(result.text_follow_data.token_raw_display_units_count,
                 static_cast<std::size_t>(8),
                 "Word-level text follow should publish one display unit per "
                 "payload byte.");
  test::AssertEq(line_token_ranges[0].token_begin_index,
                 static_cast<std::size_t>(0),
                 "First lyric line should begin at the first token.");
  test::AssertEq(line_token_ranges[0].token_count, static_cast<std::size_t>(2),
                 "Single lyric line should cover both word tokens.");
  test::AssertEq(
      line_raw_segments[0].byte_count, static_cast<std::size_t>(8),
  ...
  test::AssertEq(
      text_raw_segments[0].byte_count, static_cast<std::size_t>(4),
  ...
  test::AssertEq(text_raw_display_units[0].token_index,
                 static_cast<std::size_t>(0),
                 "First display unit should belong to the first token.");
  test::AssertEq(text_raw_display_units[0].byte_index_within_token,
                 static_cast<std::size_t>(0),
                 "First display unit should start at the first token byte.");
  test::AssertEq(
      text_raw_display_units[3].byte_index_within_token,
      static_cast<std::size_t>(3),
      "First token display units should advance one byte at a time.");
  test::AssertEq(
      text_raw_display_units[4].token_index, static_cast<std::size_t>(1),
      "Display units should switch token ownership at the token boundary.");
  test::AssertEq(
      text_raw_display_units[4].byte_index_within_token,
      static_cast<std::size_t>(0),
...
void TestApiStructuredEncodePublishesCjkTextFollow() {
  ...
  std::array<bag_text_follow_raw_segment_entry, 32> text_raw_segments{};
  std::array<bag_text_follow_raw_display_unit_entry, 32>
      text_raw_display_units{};
  std::array<bag_text_follow_lyric_line_entry, 32> lyric_line_entries{};
  std::array<bag_text_follow_line_token_range_entry, 32> line_token_ranges{};
  std::array<bag_text_follow_line_raw_segment_entry, 32> line_raw_segments{};
  ...
  test::AssertEq(
      bag_encode_text_with_follow(&encoder_config,
                                  test::Utf8Literal(u8"神机").c_str(), &result),
      BAG_OK, "Structured encode should succeed for CJK text follow.");
  const auto tokens = SplitLineSeparatedTokens(std::string(
      text_tokens_buffer.data(), result.text_follow_data.text_tokens_size));
  const auto lyric_lines = SplitLineSeparatedTokens(std::string(
      lyric_lines_buffer.data(), result.text_follow_data.lyric_lines_size));
  test::AssertEq(tokens.size(), static_cast<std::size_t>(2),
                 "CJK text should tokenize by character.");
  test::AssertEq(tokens[0], test::Utf8Literal(u8"神"),
                 "First CJK token should preserve the first character.");
  test::AssertEq(tokens[1], test::Utf8Literal(u8"机"),
                 "Second CJK token should preserve the second character.");
  test::AssertEq(
      result.text_follow_data.text_token_timeline_count, tokens.size(),
  ...
  test::AssertEq(result.text_follow_data.token_raw_display_units_count,
                 ...
                 "CJK text follow should publish one display unit per UTF-8 "
                 "payload byte.");
  test::AssertEq(lyric_lines.size(), static_cast<std::size_t>(1),
                 "Short CJK text should stay on a single lyric line.");
  test::AssertEq(lyric_lines.front(), test::Utf8Literal(u8"神机"),
                 "Lyric line should preserve compact CJK formatting.");
  test::AssertEq(line_token_ranges[0].token_count, static_cast<std::size_t>(2),
                 "Single CJK lyric line should cover both character tokens.");
  test::AssertEq(
      text_raw_segments[0].byte_count, static_cast<std::size_t>(3),
      "Each UTF-8 CJK token should map to its three-byte payload span.");
  test::AssertEq(
      text_raw_segments[1].byte_offset, static_cast<std::size_t>(3),
      "Second UTF-8 CJK token should start after the first byte span.");
  test::AssertEq(
      std::string(
          text_character_text_buffer.data() + text_characters[0].text_offset,
          text_characters[0].text_size),
      test::Utf8Literal(u8"神"),
      "Character text offsets should resolve the first UTF-8 CJK glyph.");
  test::AssertEq(
      text_raw_display_units[2].token_index, static_cast<std::size_t>(0),
      "First CJK token should own its third byte-sized display unit.");
  test::AssertEq(
      text_raw_display_units[3].token_index, static_cast<std::size_t>(1),
...
void TestApiStructuredEncodePublishesCharacterAndByteBoundariesAcrossScripts() {
  ...
  const auto run_case = [&](bag_transport_mode mode, const std::string& input,
    ...
    test::AssertEq(result.text_follow_data.text_characters_status,
                   BAG_DECODE_CONTENT_STATUS_OK,
    ...
    test::AssertEq(result.text_follow_data.text_characters_count,
                   expected_character_count,
                   "Character-level text follow should publish the expected "
                   "number of characters.");
    test::AssertEq(result.text_follow_data.token_raw_display_units_count,
                   expected_raw_display_unit_count,
                   "Byte-level display units should still align one entry per "
                   "payload byte.");

    const auto tokens = SplitLineSeparatedTokens(std::string(
        text_tokens_buffer.data(), result.text_follow_data.text_tokens_size));
    test::AssertEq(tokens.size(), expected_tokens.size(),
                   "Tokenization should remain stable while publishing "
                   "character metadata.");
  ...
  run_case(
      BAG_TRANSPORT_PRO, "hello world", {"hello", "world"},
      static_cast<std::size_t>(11), static_cast<std::size_t>(11),
      [&](const auto& text_characters, const auto& text_raw_display_units,
          const bag_encode_result& result,
          const auto& text_character_text_buffer) {
        test::AssertEq(result.text_follow_data.text_characters_count,
                       static_cast<std::size_t>(11),
                       "Latin word tokens should publish visible letters plus "
                       "the attached separator.");
        test::AssertEq(text_characters[0].text_size,
                       static_cast<std::size_t>(1),
                       "Latin characters should keep single-byte text spans.");
        test::AssertEq(text_characters[5].kind,
                       BAG_TEXT_FOLLOW_CHARACTER_KIND_SPACE,
                       "Trailing spaces should publish their own separator "
                       "character kind.");
        test::AssertEq(std::string(text_character_text_buffer.data() +
                                       text_characters[5].text_offset,
                                   text_characters[5].text_size),
                       std::string(" "),
                       "Separator characters should roundtrip through the "
                       "joined character text buffer.");
        test::AssertEq(text_raw_display_units[0].character_byte_count,
                       static_cast<std::size_t>(1),
                       "ASCII bytes should report single-byte characters.");
        test::AssertEq(text_raw_display_units[0].byte_index_within_character,
                       static_cast<std::size_t>(0),
                       "ASCII characters should start at the first byte within "
                       "their character.");
        test::AssertEq(text_raw_display_units[0].is_character_start, 1,
                       "ASCII bytes should mark character starts.");
        test::AssertEq(text_raw_display_units[0].is_character_end, 1,
                       "ASCII bytes should also mark character ends.");
        test::AssertEq(text_raw_display_units[5].token_index,
                       static_cast<std::size_t>(0),
                       "The separator byte should remain attached to the first "
                       "token payload span.");
        test::AssertEq(text_raw_display_units[5].character_byte_count,
                       static_cast<std::size_t>(1),
                       "The separator byte should still publish a stable "
                       "single-byte boundary.");
      });

  run_case(
      BAG_TRANSPORT_ULTRA, test::Utf8Literal(u8"中文测试"),
      {
      ...
      static_cast<std::size_t>(4), static_cast<std::size_t>(12),
      [&](const auto& text_characters, const auto& text_raw_display_units,
          const bag_encode_result&, const auto&) {
        test::AssertEq(
            text_characters[0].byte_count, static_cast<std::size_t>(3),
        ...
        test::AssertEq(text_raw_display_units[2].byte_index_within_character,
                       static_cast<std::size_t>(2),
                       "The third UTF-8 byte should complete the character.");
        test::AssertEq(text_raw_display_units[0].is_character_start, 1,
                       "The first CJK byte should mark character start.");
        test::AssertEq(text_raw_display_units[2].is_character_end, 1,
                       "The third CJK byte should mark character end.");
      });

  run_case(
      BAG_TRANSPORT_ULTRA, test::Utf8Literal(u8"A中B"),
      {"A", test::Utf8Literal(u8"中"), "B"}, static_cast<std::size_t>(3),
      static_cast<std::size_t>(5),
      [&](const auto& text_characters, const auto& text_raw_display_units,
          const bag_encode_result&, const auto&) {
        test::AssertEq(
            text_characters[0].byte_count, static_cast<std::size_t>(1),
            "Mixed text should preserve single-byte ASCII character spans.");
        test::AssertEq(
            text_characters[1].byte_count, static_cast<std::size_t>(3),
            "Mixed text should preserve three-byte UTF-8 CJK spans.");
        test::AssertEq(text_characters[2].byte_count,
                       static_cast<std::size_t>(1),
                       "Mixed text should preserve the trailing ASCII span.");
        test::AssertEq(text_raw_display_units[1].character_byte_count,
                       static_cast<std::size_t>(3),
                       "The CJK byte range in mixed text should still publish "
                       "a three-byte span.");
        test::AssertEq(text_raw_display_units[3].byte_index_within_character,
                       static_cast<std::size_t>(2),
                       "The final UTF-8 byte should stay aligned inside the "
                       "mixed-script character.");
        test::AssertEq(
            text_raw_display_units[4].token_index, static_cast<std::size_t>(2),
...
void TestApiStructuredEncodeDetachesPunctuationAcrossScripts() {
...

  {
    ...
    const auto lyric_lines = SplitLineSeparatedTokens(std::string(
        lyric_lines_buffer.data(), result.text_follow_data.lyric_lines_size));
    test::AssertEq(
        tokens.size(), static_cast<std::size_t>(4),
    ...
    test::AssertEq(
        lyric_lines.front(), std::string("HELLO, WORLD!"),
        "Lyric line text should preserve original punctuation placement.");
    test::AssertEq(
        line_token_ranges[0].token_count, static_cast<std::size_t>(4),
...

  {
    ...
    const auto lyric_lines = SplitLineSeparatedTokens(std::string(
        lyric_lines_buffer.data(), result.text_follow_data.lyric_lines_size));
    test::AssertEq(
        tokens.size(), static_cast<std::size_t>(8),
    ...
    test::AssertEq(tokens[5], test::Utf8Literal(u8"启"),
                   "CJK text should continue to tokenize by character between "
                   "punctuation marks.");
    ...
    test::AssertEq(
        lyric_lines.front(), input,
        "Lyric line text should preserve original CJK punctuation placement.");
    test::AssertEq(line_token_ranges[0].token_count,
                   static_cast<std::size_t>(8),
                   "Lyric line token range should include detached CJK "
                   "punctuation tokens.");
    bag_free_encode_result(&result);
  }
}
...
void TestApiStructuredDecodePublishesTextFollowAndKeepsRawOnlyFallback() {
  ...
  const auto decoder_config =
      MakeDecoderConfig(config_case, BAG_TRANSPORT_FLASH);
  bag_pcm16_result pcm{};
  test::AssertEq(bag_encode_text(&encoder_config, "RITUAL. WORD", &pcm), BAG_OK,
  ...
  test::AssertEq(bag_push_pcm(decoder, pcm.samples, pcm.sample_count, 0),
                 BAG_OK, "PCM push should succeed for structured text follow.");
  ...
  const auto lyric_lines = SplitLineSeparatedTokens(std::string(
      lyric_lines_buffer.data(), result.text_follow_data.lyric_lines_size));
  test::AssertTrue(result.text_follow_data.available != 0,
                   "Structured decode should publish text follow.");
  test::AssertEq(tokens.size(), static_cast<std::size_t>(3),
                 "Decoded text should reuse the tokenized text follow.");
  test::AssertEq(lyric_lines.size(), static_cast<std::size_t>(2),
                 "Decoded text with short-phrase punctuation should split into "
                 "lyric lines.");
  test::AssertEq(result.text_follow_data.text_token_timeline_count,
                 tokens.size(),
  ...
  for (std::size_t index = 0;
       index < result.text_follow_data.text_token_timeline_count; ++index) {
    test::AssertTrue(
    ...
    test::AssertTrue(
        text_entries[index].start_sample + text_entries[index].sample_count <=
            result.follow_data.payload_begin_sample +
                result.follow_data.payload_sample_count,
        "Text-follow entries should not extend beyond the payload sample "
        "range.");
    test::AssertEq(
        text_raw_segments[index].start_sample, text_entries[index].start_sample,
        "Token raw segments should share the token follow start sample.");
    test::AssertEq(
        text_raw_segments[index].sample_count, text_entries[index].sample_count,
        "Token raw segments should share the token follow sample count.");
  }
  std::size_t display_unit_cursor = 0;
  for (std::size_t index = 0;
       index < result.text_follow_data.token_raw_segments_count; ++index) {
    for (std::size_t byte_index = 0;
         byte_index < text_raw_segments[index].byte_count; ++byte_index) {
      test::AssertEq(
          text_raw_display_units[display_unit_cursor].token_index, index,
      ...
      test::AssertEq(
          ...
          byte_index,
          "Decoded display units should expose the byte index within the "
          "token.");
      ++display_unit_cursor;
    }
  }
  ...
  test::AssertEq(
      result.text_follow_data.lyric_line_timeline_count, lyric_lines.size(),
      "Structured decode should publish one timeline entry per lyric line.");
  test::AssertEq(
      result.text_follow_data.line_token_ranges_count, lyric_lines.size(),
      "Structured decode should publish one token range per lyric line.");
  test::AssertEq(
      result.text_follow_data.line_raw_segments_count, lyric_lines.size(),
      "Structured decode should publish one raw segment per lyric line.");
  bag_free_pcm16_result(&pcm);

  bag_decoder* raw_only_decoder = nullptr;
  const auto raw_only_config =
      MakeDecoderConfig(config_case, BAG_TRANSPORT_PRO);
  test::AssertEq(bag_create_decoder(&raw_only_config, &raw_only_decoder),
                 BAG_OK, "Raw-only decoder creation should succeed.");
  const std::array<std::int16_t, 2> invalid_pcm = {0, 0};
  test::AssertEq(
      bag_push_pcm(raw_only_decoder, invalid_pcm.data(), invalid_pcm.size(), 0),
      BAG_OK, "Raw-only decoder should accept the synthetic PCM block.");
  ...
  raw_only_result.text_follow_data.text_tokens_buffer =
      text_tokens_buffer.data();
  raw_only_result.text_follow_data.text_tokens_buffer_size =
      text_tokens_buffer.size();
  raw_only_result.text_follow_data.lyric_lines_buffer =
      lyric_lines_buffer.data();
  raw_only_result.text_follow_data.lyric_lines_buffer_size =
      lyric_lines_buffer.size();
  raw_only_result.text_follow_data.text_token_timeline_buffer =
      text_entries.data();
  raw_only_result.text_follow_data.text_token_timeline_buffer_count =
      text_entries.size();
  raw_only_result.text_follow_data.token_raw_segments_buffer =
      text_raw_segments.data();
  raw_only_result.text_follow_data.token_raw_segments_buffer_count =
      text_raw_segments.size();
  raw_only_result.text_follow_data.token_raw_display_units_buffer =
      text_raw_display_units.data();
  raw_only_result.text_follow_data.token_raw_display_units_buffer_count =
      text_raw_display_units.size();
  raw_only_result.text_follow_data.lyric_line_timeline_buffer =
      lyric_line_entries.data();
  raw_only_result.text_follow_data.lyric_line_timeline_buffer_count =
      lyric_line_entries.size();
  raw_only_result.text_follow_data.line_token_ranges_buffer =
      line_token_ranges.data();
  raw_only_result.text_follow_data.line_token_ranges_buffer_count =
      line_token_ranges.size();
  raw_only_result.text_follow_data.line_raw_segments_buffer =
      line_raw_segments.data();
  raw_only_result.text_follow_data.line_raw_segments_buffer_count =
      line_raw_segments.size();
  ...
  raw_only_result.follow_data.binary_group_timeline_buffer =
      binary_entries.data();
  raw_only_result.follow_data.binary_group_timeline_buffer_count =
      binary_entries.size();
  const auto raw_only_code =
      bag_poll_decode_result(raw_only_decoder, &raw_only_result);
  bag_destroy_decoder(raw_only_decoder);

  if (raw_only_code == BAG_OK && raw_only_result.raw_payload_available != 0 &&
      raw_only_result.text_decode_status ==
          BAG_DECODE_CONTENT_STATUS_INVALID_TEXT_PAYLOAD) {
    test::AssertTrue(
...
void TestApiStructuredEncodePublishesShortPhraseLyricLines() {
  ...
  test::AssertEq(
      bag_encode_text_with_follow(&encoder_config, input.c_str(), &result),
      BAG_OK, "Structured encode should succeed for lyric-line tests.");

  const auto lyric_lines = SplitLineSeparatedTokens(std::string(
      lyric_lines_buffer.data(), result.text_follow_data.lyric_lines_size));
  test::AssertTrue(lyric_lines.size() >= static_cast<std::size_t>(2),
                   "Lyric line builder should honor explicit newlines and "
                   "short-phrase breaks.");
  test::AssertEq(
      lyric_lines.front(), std::string("ASH BELL."),
  ...
  for (std::size_t index = 0;
       index < result.text_follow_data.lyric_line_timeline_count; ++index) {
    test::AssertTrue(lyric_line_entries[index].sample_count > 0,
                     "Each lyric line should span a non-empty sample range.");
    test::AssertTrue(line_raw_segments[index].byte_count > 0,
...
void TestApiLegacyTextPollingStaysCompatibleWhenStructuredDecodeHasRawOnlyValue() {
  ...
  test::AssertEq(bag_create_decoder(&decoder_config, &structured_decoder),
                 BAG_OK, "Structured decoder creation should succeed.");
  const std::array<std::int16_t, 2> invalid_pcm = {0, 0};
  test::AssertEq(bag_push_pcm(structured_decoder, invalid_pcm.data(),
                              invalid_pcm.size(), 0),
                 BAG_OK,
  ...
  const auto structured_code =
      bag_poll_decode_result(structured_decoder, &decode_result);
  bag_destroy_decoder(structured_decoder);

  bag_decoder* legacy_decoder = nullptr;
  test::AssertEq(bag_create_decoder(&decoder_config, &legacy_decoder), BAG_OK,
                 "Legacy decoder creation should succeed.");
  test::AssertEq(
      bag_push_pcm(legacy_decoder, invalid_pcm.data(), invalid_pcm.size(), 0),
      BAG_OK, "Legacy decoder should accept the synthetic PCM block.");
  ...
  if (structured_code == BAG_OK && decode_result.raw_payload_available != 0 &&
      decode_result.text_decode_status ==
          BAG_DECODE_CONTENT_STATUS_INVALID_TEXT_PAYLOAD) {
    test::AssertEq(legacy_code, BAG_INTERNAL,
                   "Legacy text polling should continue reporting an internal "
                   "failure for raw-only results.");
  }
}
...
void TestApiPollResultBufferBoundaries() {
...

  {
    ...
    test::AssertEq(bag_poll_result(decoder, &result), BAG_OK,
                   "Polling with a null output buffer should still succeed.");
    test::AssertEq(result.text_size, text.size(),
                   "Null-buffer polling should still publish full text size.");
    test::AssertEq(
        result.complete, 1,
        "Null-buffer polling should still mark the result complete.");
    test::AssertEq(result.mode, BAG_TRANSPORT_PRO,
                   "Null-buffer polling should still preserve mode.");
    bag_destroy_decoder(decoder);
  }

  {
    bag_decoder* decoder = nullptr;
    test::AssertEq(
        bag_create_decoder(&decoder_config, &decoder), BAG_OK,
    ...
    test::AssertEq(
        bag_poll_result(decoder, &result), BAG_OK,
        "Polling with a zero-size output buffer should still succeed.");
    test::AssertEq(std::string(sentinel), std::string("keep"),
                   "Zero-size output buffers should remain untouched.");
    test::AssertEq(
        result.text_size, text.size(),
        "Zero-size-buffer polling should still publish full text size.");
    test::AssertEq(
        result.complete, 1,
        "Zero-size-buffer polling should still mark the result complete.");
    bag_destroy_decoder(decoder);
  }

  {
    ...
    test::AssertEq(result.text_size, text.size(),
                   "Small-buffer polling should still publish full text size.");
    test::AssertEq(
        result.complete, 1,
        "Small-buffer polling should still mark the result complete.");
    bag_destroy_decoder(decoder);
  }
...
void TestApiDecoderInputAndRepeatedPollBoundaries() {
  ...
  test::AssertEq(bag_poll_result(decoder, nullptr), BAG_INVALID_ARGUMENT,
                 "Polling with a null text result target should be rejected.");
  test::AssertEq(bag_poll_result(decoder, &result), BAG_NOT_READY,
                 "Polling before any PCM push should return not ready.");
  test::AssertEq(result.text_size, static_cast<std::size_t>(0),
                 "Not-ready polling should clear text size.");
  test::AssertEq(result.complete, 0,
                 "Not-ready polling should clear completion state.");
  test::AssertEq(result.confidence, 0.0f,
                 "Not-ready polling should clear confidence.");
  test::AssertEq(result.mode, BAG_TRANSPORT_FLASH,
                 "Not-ready polling should reset mode to the API default.");
  test::AssertEq(result.buffer[0], '\0',
                 "Not-ready polling should clear the first output byte.");

  test::AssertEq(
      bag_push_pcm(decoder, nullptr, pcm.sample_count, 0), BAG_INVALID_ARGUMENT,
      "Null PCM pointers with non-zero sample counts should be rejected.");
  test::AssertEq(bag_push_pcm(decoder, pcm.samples, 0, 0), BAG_INVALID_ARGUMENT,
                 "Zero-length PCM pushes should be rejected.");

  test::AssertEq(bag_push_pcm(decoder, pcm.samples, pcm.sample_count, 0),
                 BAG_OK, "Valid PCM push should succeed.");
  test::AssertEq(bag_poll_result(decoder, &result), BAG_OK,
                 "Polling after a valid push should succeed.");
  test::AssertEq(std::string(result.buffer, result.text_size), text,
                 "Successful polling should preserve the decoded text.");
  test::AssertEq(result.complete, 1,
                 "Successful polling should mark the result complete.");
  test::AssertEq(result.mode, BAG_TRANSPORT_FLASH,
                 "Successful polling should preserve the configured mode.");

  result.buffer[0] = 'Y';
  test::AssertEq(bag_poll_result(decoder, &result), BAG_NOT_READY,
                 "Polling again after consuming the pending result should "
                 "return not ready.");
  test::AssertEq(result.text_size, static_cast<std::size_t>(0),
                 "Repeated not-ready polling should clear text size.");
  test::AssertEq(result.complete, 0,
                 "Repeated not-ready polling should clear completion state.");
  test::AssertEq(
      result.buffer[0], '\0',
      "Repeated not-ready polling should clear the first output byte.");
  ...
  test::AssertEq(result.buffer[0], '\0',
                 "Polling after reset should clear the first output byte.");
...
void TestApiFreePcmResultIsIdempotent() {
  ...
  bag_pcm16_result pcm{};
  test::AssertEq(bag_encode_text(&encoder_config, "free", &pcm), BAG_OK,
                 "PCM result should be allocated for a successful encode.");
  test::AssertTrue(pcm.samples != nullptr,
                   "PCM result buffer should be allocated.");
  test::AssertTrue(pcm.sample_count > 0,
                   "PCM result sample count should be non-zero.");

  bag_free_pcm16_result(&pcm);
  test::AssertTrue(pcm.samples == nullptr,
                   "PCM result samples should clear after free.");
  test::AssertEq(pcm.sample_count, static_cast<size_t>(0),
                 "PCM result count should clear after free.");

  bag_free_pcm16_result(&pcm);
  test::AssertTrue(pcm.samples == nullptr,
                   "PCM free should remain safe on a cleared result.");
  test::AssertEq(pcm.sample_count, static_cast<size_t>(0),
...
void TestApiVersionMatchesRelease() {
  ...
  test::AssertEq(std::string(version), std::string(test::kExpectedCoreVersion),
                 "C API should expose the current release version.");
}

void TestApiValidationHelpers() {
  bag_transport_mode mode = BAG_TRANSPORT_FLASH;
  test::AssertTrue(bag_try_parse_transport_mode("flash", &mode) != 0,
                   "Mode parser should accept flash.");
  test::AssertEq(mode, BAG_TRANSPORT_FLASH,
                 "Mode parser should map flash correctly.");
  test::AssertTrue(bag_try_parse_transport_mode("pro", &mode) != 0,
                   "Mode parser should accept pro.");
  test::AssertEq(mode, BAG_TRANSPORT_PRO,
                 "Mode parser should map pro correctly.");
  test::AssertTrue(bag_try_parse_transport_mode("ultra", &mode) != 0,
                   "Mode parser should accept ultra.");
  test::AssertEq(mode, BAG_TRANSPORT_ULTRA,
                 "Mode parser should map ultra correctly.");
  test::AssertTrue(bag_try_parse_transport_mode("mini", &mode) != 0,
                   "Mode parser should accept mini.");
  test::AssertEq(mode, BAG_TRANSPORT_MINI,
                 "Mode parser should map mini correctly.");
  test::AssertTrue(bag_try_parse_transport_mode("warp", &mode) == 0,
                   "Mode parser should reject unsupported values.");

  test::AssertEq(std::string(bag_transport_mode_name(BAG_TRANSPORT_PRO)),
                 std::string("pro"),
                 "Mode name helper should return the stable CLI spelling.");
  test::AssertEq(std::string(bag_transport_mode_name(BAG_TRANSPORT_MINI)),
                 std::string("mini"),
  ...
  test::AssertContains(
      bag_validation_issue_message(BAG_VALIDATION_PRO_ASCII_ONLY), "ASCII",
      "Validation helper message should explain the ASCII-only rule.");
  test::AssertEq(bag_validate_encode_request(
                     &pro_config, test::BuildTooLongProCorpus().c_str()),
                 BAG_VALIDATION_OK,
                 "Validation helper should reflect that pro no longer inherits "
                 "the compat frame limit.");

  auto ultra_config = MakeEncoderConfig(config_case, BAG_TRANSPORT_ULTRA);
  test::AssertEq(bag_validate_encode_request(
                     &ultra_config, test::BuildTooLongUltraCorpus().c_str()),
                 BAG_VALIDATION_OK,
                 "Validation helper should reflect that ultra no longer "
                 "inherits the compat frame limit.");
  ...
  test::AssertEq(bag_validate_encode_request(&mini_config, ""),
                 BAG_VALIDATION_EMPTY_TEXT,
                 "Validation helper should distinguish truly empty text from "
                 "encodable whitespace.");
  test::AssertContains(
      bag_validation_issue_message(BAG_VALIDATION_MINI_MORSE_ONLY), "Morse",
      "Validation helper message should explain the mini Morse-only rule.");
  test::AssertContains(
      bag_validation_issue_message(BAG_VALIDATION_EMPTY_TEXT), "empty",
      "Validation helper message should explain the empty-text rule.");

  auto invalid_decoder =
      MakeDecoderConfig(config_case, static_cast<bag_transport_mode>(99));
  test::AssertEq(bag_validate_decoder_config(&invalid_decoder),
  ...
  invalid_flash_encoder.flash_signal_profile =
      static_cast<bag_flash_signal_profile>(99);
  test::AssertEq(
  ...
  invalid_flash_decoder.flash_voicing_flavor =
      static_cast<bag_flash_voicing_flavor>(99);
  test::AssertEq(bag_validate_decoder_config(&invalid_flash_decoder),
                 BAG_VALIDATION_INVALID_FLASH_VOICING_FLAVOR,
                 "Decoder validation helper should reject unsupported flash "
                 "voicing flavors.");
  test::AssertContains(
      bag_validation_issue_message(BAG_VALIDATION_INVALID_FLASH_SIGNAL_PROFILE),
      "signal profile",
      "Validation helper message should explain the flash signal-profile "
      "failure.");
  test::AssertContains(
      bag_validation_issue_message(BAG_VALIDATION_INVALID_FLASH_VOICING_FLAVOR),
      "voicing flavor",
      "Validation helper message should explain the flash voicing-flavor "
      "failure.");
  test::AssertContains(
      bag_error_code_message(BAG_INTERNAL), "Internal",
...
void RegisterApiSyncTests(test::Runner& runner) {
  runner.Add("Api.FlashRoundTripAcrossCorpusAndConfigs",
             TestApiFlashRoundTripAcrossCorpusAndConfigs);
  runner.Add("Api.ProRoundTripAcrossCorpusAndConfigs",
             TestApiProRoundTripAcrossCorpusAndConfigs);
  runner.Add("Api.UltraRoundTripAcrossCorpusAndConfigs",
  ...
  runner.Add("Api.CreateDecoderRejectsInvalidArguments",
             TestApiCreateDecoderRejectsInvalidArguments);
  ...
  runner.Add("Api.ChunkedPushRoundTripAcrossModes",
             TestApiChunkedPushRoundTripAcrossModes);
  runner.Add("Api.DecodeResultPublishesRawPayloadAcrossModes",
             TestApiDecodeResultPublishesRawPayloadAcrossModes);
  runner.Add("Api.DecodeResultBufferBoundaries",
             TestApiDecodeResultBufferBoundaries);
  runner.Add("Api.StructuredEncodePublishesFollowAcrossModes",
             TestApiStructuredEncodePublishesFollowAcrossModes);
  runner.Add("Api.EncodeAndDecodeFollowStayAligned",
             TestApiEncodeAndDecodeFollowStayAligned);
  runner.Add("Api.BuildEncodeFollowDataMatchesStructuredEncode",
             TestApiBuildEncodeFollowDataMatchesStructuredEncode);
  runner.Add("Api.FlashFollowTimingRespectsStyleRules",
             TestApiFlashFollowTimingRespectsStyleRules);
  runner.Add("Api.StructuredEncodePublishesWordLevelTextFollow",
             TestApiStructuredEncodePublishesWordLevelTextFollow);
  runner.Add("Api.StructuredEncodePublishesCjkTextFollow",
             TestApiStructuredEncodePublishesCjkTextFollow);
  runner.Add(
      "Api.StructuredEncodePublishesCharacterAndByteBoundariesAcrossScripts",
  ...
  runner.Add(
      "Api.LegacyTextPollingStaysCompatibleWhenStructuredDecodeHasRawOnlyValue",
      TestApiLegacyTextPollingStaysCompatibleWhenStructuredDecodeHasRawOnlyValue);
  runner.Add("Api.PollResultBufferBoundaries",
             TestApiPollResultBufferBoundaries);
  runner.Add("Api.DecoderInputAndRepeatedPollBoundaries",
             TestApiDecoderInputAndRepeatedPollBoundaries);
