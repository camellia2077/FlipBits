...
bag_encoder_config MakeEncoderConfig(
    const test::ConfigCase& config_case, bag_transport_mode mode,
...
std::size_t RoundHalfUpFrameScale(int frame_samples, int numerator,
  ...
  return frame_samples > 0
             ? static_cast<std::size_t>(
                   (frame_samples * numerator + (denominator / 2)) /
                   denominator)
...
bool IsLikelyUtf8CodePointBoundaryAfter(const std::string& text,
                                        std::size_t byte_index) {
  return byte_index + static_cast<std::size_t>(1) >= text.size() ||
         !IsUtf8ContinuationByte(static_cast<unsigned char>(
             text[byte_index + static_cast<std::size_t>(1)]));
}

bool EndsWithUtf8Bytes(const std::string& text, std::size_t byte_index,
  ...
  return static_cast<unsigned char>(
             text[byte_index - static_cast<std::size_t>(2)]) == first &&
         static_cast<unsigned char>(
             text[byte_index - static_cast<std::size_t>(1)]) == second &&
         static_cast<unsigned char>(text[byte_index]) == third;
}

std::size_t LitanyPauseSlotCountAfterByte(const std::string& text,
                                          std::size_t byte_index) {
...
std::size_t LitanyPauseSlotCount(const std::string& text) {
  ...
  for (std::size_t byte_index = 0; byte_index < text.size(); ++byte_index) {
    pause_slots += static_cast<std::size_t>(14) +
                   LitanyPauseSlotCountAfterByte(text, byte_index);
  }
  return pause_slots;
}

std::size_t LitanyPauseSampleCount(const std::string& text,
                                   std::size_t silence_slot_samples) {
  return LitanyPauseSlotCount(text) * silence_slot_samples;
}

std::size_t ZealBitSampleCount(std::size_t frame_samples,
                               std::size_t bit_position,
                               bool force_burst_after_pause) {
  if (frame_samples == 0) {
    return 0;
  }
...
std::size_t ZealPauseSlotCountAfterByte(const std::string& text,
                                        std::size_t byte_index) {
...
std::size_t ZealPayloadSampleCount(const std::string& text,
  ...
  for (std::size_t byte_index = 0; byte_index < text.size(); ++byte_index) {
    for (int bit_index = 0; bit_index < 8; ++bit_index) {
      ...
      sample_count += ZealBitSampleCount(frame_samples, bit_position,
                                         force_burst_after_pause);
      force_burst_after_pause = false;
    }
    const std::size_t pause_slots =
        ZealPauseSlotCountAfterByte(text, byte_index);
...
std::size_t VoidBitSampleCount(std::size_t frame_samples,
                               std::size_t bit_position, int bit_index) {
  if (frame_samples == 0) {
    return 0;
  }
  if (bit_index == 7) {
    return std::max(static_cast<std::size_t>(1),
                    frame_samples * static_cast<std::size_t>(11) /
                        static_cast<std::size_t>(4));
  }
...
std::size_t VoidPauseSlotCountAfterBit(std::size_t bit_position,
                                       std::size_t byte_index, int bit_index) {
  if (bit_index == 7) {
...
std::size_t VoidPayloadSampleCount(const std::string& text,
                                   std::size_t frame_samples) {
  std::size_t sample_count = 0;
  for (std::size_t byte_index = 0; byte_index < text.size(); ++byte_index) {
    for (int bit_index = 0; bit_index < 8; ++bit_index) {
      const std::size_t bit_position =
          ...
          static_cast<std::size_t>(bit_index);
      sample_count +=
          VoidBitSampleCount(frame_samples, bit_position, bit_index);
      sample_count +=
          VoidPauseSlotCountAfterBit(bit_position, byte_index, bit_index) *
          frame_samples;
    }
  }
  return sample_count;
...
std::size_t ExpectedFlashSampleCount(
  ...
  if (flash_signal_profile == BAG_FLASH_SIGNAL_PROFILE_STANDARD) {
    payload_samples_per_bit =
        std::max(static_cast<std::size_t>(1), frame_samples *
  ...
  const std::size_t trailing_nonpayload_samples =
      flash_voicing_flavor == BAG_FLASH_VOICING_FLAVOR_LITANY
          ...
          : frame_samples * static_cast<std::size_t>(3);
  const bool uses_litany_pauses =
      flash_voicing_flavor == BAG_FLASH_VOICING_FLAVOR_LITANY;
  const bool uses_zeal_layout =
      flash_voicing_flavor == BAG_FLASH_VOICING_FLAVOR_ZEAL;
  const bool uses_void_layout =
      flash_voicing_flavor == BAG_FLASH_VOICING_FLAVOR_VOID;
  return (uses_zeal_layout   ? ZealPayloadSampleCount(text, frame_samples)
          : uses_void_layout ? VoidPayloadSampleCount(text, frame_samples)
                             : text.size() * static_cast<std::size_t>(8) *
                                   payload_samples_per_bit) +
         (uses_litany_pauses
              ? LitanyPauseSampleCount(text,
                                       payload_samples_per_litany_silence_slot)
              : static_cast<std::size_t>(0)) +
         leading_nonpayload_samples + trailing_nonpayload_samples;
}
...
DecodeResult DecodeViaApi(const bag_decoder_config& config,
  ...
  test::AssertTrue(decoder != nullptr,
                   "Decoder should not be null after creation.");
  ...
  out.raw_bytes_hex.assign(raw_bytes_hex_buffer.data(),
                           result.raw_bytes_hex_size);
  out.raw_bits_binary.assign(raw_bits_binary_buffer.data(),
...
DecodeResult DecodeViaApiInChunks(const bag_decoder_config& config,
  ...
  bag_decoder* decoder = nullptr;
  const auto create_code = bag_create_decoder(&config, &decoder);
  test::AssertEq(create_code, BAG_OK,
                 "Chunked decoder creation should succeed.");
  test::AssertTrue(decoder != nullptr,
                   "Chunked decoder should not be null after creation.");

  const std::size_t normalized_chunk_sample_count =
      std::max<std::size_t>(1, chunk_sample_count);
  for (std::size_t offset = 0; offset < pcm.sample_count;
       offset += normalized_chunk_sample_count) {
    const std::size_t chunk_size =
        std::min(normalized_chunk_sample_count, pcm.sample_count - offset);
    const auto push_code =
        bag_push_pcm(decoder, pcm.samples + offset, chunk_size,
...
void AssertPcmResultsEqual(const bag_pcm16_result& lhs,
  ...
  test::AssertEq(lhs.sample_count, rhs.sample_count,
                 message + " sample count should match.");
...
void AssertRoundTripAcrossCorpus(const std::vector<test::CorpusCase>& corpus,
  ...
  for (const auto& config_case : test::ConfigCases()) {
    ...
    for (const auto& corpus_case : corpus) {
      ...
      test::AssertEq(encode_code, BAG_OK,
                     "C API encode should succeed across corpus and configs.");

      const auto decoded = DecodeViaApi(decoder_config, pcm);
      test::AssertEq(decoded.code, BAG_OK,
                     "Polling should succeed after valid push.");
      test::AssertEq(decoded.text, corpus_case.text,
                     "API roundtrip should preserve original text.");
      test::AssertEq(decoded.mode, mode,
                     "API result mode should match the configured mode.");
