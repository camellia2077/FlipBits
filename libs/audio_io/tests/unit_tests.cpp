...
constexpr std::size_t kMetadataInputSourceKindOffset =
    kWavChunkPayloadOffset + 4;
constexpr std::size_t kMetadataHasMiniSpeedStyleOffset =
    kWavChunkPayloadOffset + 5;
constexpr std::size_t kMetadataMiniSpeedStyleOffset =
    kWavChunkPayloadOffset + 6;
...
std::vector<std::uint32_t> MakeValidSegmentSampleCounts(
  ...
  const std::uint32_t first =
      std::max<std::uint32_t>(1u, pcm_sample_count / 3u);
  const std::uint32_t remaining_after_first = pcm_sample_count - first;
  const std::uint32_t second =
      std::max<std::uint32_t>(1u, remaining_after_first / 2u);
  const std::uint32_t third = pcm_sample_count - first - second;
  return {first, second, third};
}

audio_io::FlipBitsAudioMetadata MakeValidMetadata(
    std::uint32_t pcm_sample_count) {
  ...
  metadata.segment_sample_counts =
      MakeValidSegmentSampleCounts(pcm_sample_count);
  metadata.app_version = kTestAppVersion;
  metadata.core_version = kTestCoreVersion;
  return metadata;
}

audio_io::FlipBitsAudioMetadata MakeValidFlashMetadata(
    std::uint32_t pcm_sample_count) {
...
std::vector<std::uint8_t> MakeMetadataWavBytes() {
  ...
  return audio_io::SerializeMonoPcm16WavWithMetadata(
      test_case.sample_rate_hz, test_case.mono_pcm,
...
void TestWavIoHeaderRoundTripContract() {
  ...
  for (const auto& test_case : test::AudioIoRoundTripCases()) {
    ...
    audio_io::WriteMonoPcm16Wav(path, test_case.sample_rate_hz,
                                test_case.mono_pcm);
    const auto read_back = audio_io::ReadMonoPcm16Wav(path);
    test::AssertAudioIoRoundTripResult(read_back, test_case,
                                       "Header audio_io boundary");
  }
}

void TestWavIoHeaderBytesRoundTripContract() {
  for (const auto& test_case : test::AudioIoRoundTripCases()) {
    const auto wav_bytes = audio_io::SerializeMonoPcm16Wav(
        test_case.sample_rate_hz, test_case.mono_pcm);
    const auto parsed = audio_io::ParseMonoPcm16Wav(wav_bytes);
    test::AssertEq(
        parsed.status, audio_io::WavPcm16Status::kOk,
        "Header bytes route should parse canonical mono PCM16 WAV bytes.");
    test::AssertAudioIoRoundTripResult(parsed.wav, test_case,
                                       "Header audio_io bytes boundary");
  }
}
...
void TestWavIoHeaderProbeRoundTripContract() {
  for (const auto& test_case : test::AudioIoRoundTripCases()) {
    ...
    test::AssertEq(
        probed.status, audio_io::WavPcm16Status::kOk,
        "Header probe should accept canonical mono PCM16 WAV bytes.");
    test::AssertEq(probed.info.sample_rate_hz, test_case.sample_rate_hz,
                   "Header probe sample rate should match.");
    test::AssertEq(probed.info.channels, 1,
                   "Header probe channel count should match.");
    test::AssertEq(probed.info.bits_per_sample, 16,
                   "Header probe bit depth should match.");
    test::AssertEq(probed.info.pcm_sample_count,
                   static_cast<std::uint64_t>(test_case.mono_pcm.size()),
                   "Header probe sample count should match.");
    test::AssertEq(probed.info.data_byte_count,
                   static_cast<std::uint64_t>(test_case.mono_pcm.size() *
                                              sizeof(std::int16_t)),
                   "Header probe data byte count should match.");
    test::AssertEq(probed.info.file_byte_count,
...
void TestWavIoHeaderReadMissingFileFails() {
  ...
  test::AssertThrows([&] { (void)audio_io::ReadMonoPcm16Wav(missing_path); },
                     "Header audio_io boundary should throw when the input "
                     "file does not exist.");
}

void TestWavIoHeaderRejectsInvalidBytes() {
  const std::vector<std::uint8_t> bad_header = {'N', 'O', 'T', 'W',
                                                'A', 'V', 'E'};
  const auto parsed = audio_io::ParseMonoPcm16Wav(bad_header);
  test::AssertEq(parsed.status, audio_io::WavPcm16Status::kInvalidHeader,
...
void TestWavIoHeaderRejectsUnsupportedStereoBytes() {
  ...
  auto wav_bytes = audio_io::SerializeMonoPcm16Wav(test_case.sample_rate_hz,
                                                   test_case.mono_pcm);
  wav_bytes[22] = 0x02;
  wav_bytes[23] = 0x00;
  wav_bytes[32] = 0x04;
  wav_bytes[33] = 0x00;
  const auto stereo_byte_rate =
      static_cast<std::uint32_t>(test_case.sample_rate_hz * 4);
  ...
  const auto parsed = audio_io::ParseMonoPcm16Wav(wav_bytes);
  test::AssertEq(parsed.status, audio_io::WavPcm16Status::kUnsupportedFormat,
...
void TestWavIoHeaderRejectsTruncatedDataBytes() {
  ...
  wav_bytes.pop_back();
  const auto parsed = audio_io::ParseMonoPcm16Wav(wav_bytes);
  test::AssertEq(parsed.status, audio_io::WavPcm16Status::kTruncatedData,
...
void TestWavIoCApiRoundTripContract() {
  for (const auto& test_case : test::AudioIoRoundTripCases()) {
    ...
    const auto encode_status = audio_io_encode_mono_pcm16_wav(
        test_case.sample_rate_hz, test_case.mono_pcm.data(),
        test_case.mono_pcm.size(), &wav_bytes);
    test::AssertEq(
        encode_status, AUDIO_IO_WAV_OK,
        "C ABI WAV encode should succeed for canonical mono PCM16 input.");
    test::AssertTrue(wav_bytes.data != nullptr,
                     "C ABI WAV encode should allocate output bytes.");
    test::AssertTrue(wav_bytes.size > 0,
                     "C ABI WAV encode should produce non-empty bytes.");

    audio_io_decoded_wav decoded{};
    const auto decode_status = audio_io_decode_mono_pcm16_wav(
        wav_bytes.data, wav_bytes.size, &decoded);
    test::AssertEq(
        decode_status, AUDIO_IO_WAV_OK,
        "C ABI WAV decode should succeed for canonical mono PCM16 WAV bytes.");
    test::AssertEq(decoded.sample_rate_hz, test_case.sample_rate_hz,
                   "C ABI decode sample rate should round-trip.");
    test::AssertEq(decoded.channels, 1,
                   "C ABI decode should preserve the mono channel count.");
    test::AssertEq(decoded.sample_count, test_case.mono_pcm.size(),
                   "C ABI decode sample count should round-trip.");
    test::AssertEq(decoded.metadata_status, AUDIO_IO_METADATA_NOT_FOUND,
                   "C ABI decode should surface missing metadata separately "
                   "for canonical WAV.");
    const bool samples_match =
        decoded.sample_count == test_case.mono_pcm.size() &&
        (decoded.sample_count == 0 ||
         std::equal(decoded.samples, decoded.samples + decoded.sample_count,
                    test_case.mono_pcm.begin()));
    test::AssertTrue(samples_match,
                     "C ABI decode PCM samples should round-trip.");
...
void TestWavIoCApiRejectsInvalidBytes() {
  const std::vector<std::uint8_t> invalid_bytes = {'N', 'O', 'T', 'W',
                                                   'A', 'V', 'E'};
  audio_io_decoded_wav decoded{};
  const auto status = audio_io_decode_mono_pcm16_wav(
      invalid_bytes.data(), invalid_bytes.size(), &decoded);
  test::AssertEq(status, AUDIO_IO_WAV_INVALID_HEADER,
                 "C ABI WAV decode should reject invalid headers.");
  test::AssertContains(
      audio_io_wav_status_message(status), "Invalid WAV header",
      "C ABI WAV status message should explain the invalid-header failure.");
  test::AssertEq(decoded.metadata_status, AUDIO_IO_METADATA_INVALID_HEADER,
                 "C ABI decode should surface metadata invalid-header when "
                 "RIFF/WAVE parsing fails early.");
  audio_io_free_decoded_wav(&decoded);
}
...
void TestWavIoCApiProbeRoundTripContract() {
  ...
  test::AssertEq(status, AUDIO_IO_WAV_OK,
                 "C ABI probe should accept metadata WAV bytes.");
  test::AssertEq(info.sample_rate_hz, test_case.sample_rate_hz,
                 "C ABI probe sample rate should match.");
  test::AssertEq(info.channels, 1, "C ABI probe channel count should match.");
  test::AssertEq(info.bits_per_sample, 16,
                 "C ABI probe bit depth should match.");
  test::AssertEq(info.pcm_sample_count,
                 static_cast<std::uint64_t>(test_case.mono_pcm.size()),
                 "C ABI probe sample count should match.");
  test::AssertEq(info.data_byte_count,
                 static_cast<std::uint64_t>(test_case.mono_pcm.size() *
                                            sizeof(std::int16_t)),
                 "C ABI probe data byte count should match.");
  test::AssertEq(info.file_byte_count,
                 static_cast<std::uint64_t>(wav_bytes.size()),
...
void TestWavIoCApiMetadataRoundTripContract() {
  const auto& test_case = MetadataRoundTripCase();
  auto metadata = MakeApiMetadataView(
      static_cast<std::uint32_t>(test_case.mono_pcm.size()));

  audio_io_byte_buffer wav_bytes{};
  const auto encode_status = audio_io_encode_mono_pcm16_wav_with_metadata(
      test_case.sample_rate_hz, test_case.mono_pcm.data(),
      test_case.mono_pcm.size(), &metadata.view, &wav_bytes);
  test::AssertEq(encode_status, AUDIO_IO_WAV_OK,
                 "C ABI WAV encode with metadata should succeed.");

  audio_io_decoded_wav decoded{};
  const auto decode_status =
      audio_io_decode_mono_pcm16_wav(wav_bytes.data, wav_bytes.size, &decoded);
  test::AssertEq(decode_status, AUDIO_IO_WAV_OK,
                 "C ABI decode should succeed for metadata WAV.");
  test::AssertEq(decoded.metadata_status, AUDIO_IO_METADATA_OK,
                 "C ABI unified decode should report metadata success.");
  test::AssertEq(decoded.metadata.version, static_cast<std::uint8_t>(7u),
                 "C ABI metadata version should round-trip.");
  test::AssertEq(decoded.metadata.mode, AUDIO_IO_METADATA_MODE_MINI,
                 "C ABI metadata mode should round-trip.");
  ...
  test::AssertEq(std::string(decoded.metadata.created_at_iso_utc.data,
                             decoded.metadata.created_at_iso_utc.size),
                 std::string("2026-03-17T09:45:00Z"),
                 "C ABI metadata timestamp should round-trip.");
  test::AssertEq(decoded.metadata.duration_ms,
                 static_cast<std::uint32_t>(4321u),
                 "C ABI metadata duration should round-trip.");
  test::AssertEq(decoded.metadata.sample_rate_hz,
                 static_cast<std::uint32_t>(44100u),
                 "C ABI metadata sample rate should round-trip.");
  test::AssertEq(decoded.metadata.frame_samples,
                 static_cast<std::uint32_t>(2205u),
                 "C ABI frame_samples should round-trip.");
  test::AssertEq(decoded.metadata.pcm_sample_count,
                 static_cast<std::uint32_t>(test_case.mono_pcm.size()),
  ...
  test::AssertEq(decoded.metadata.segment_count, static_cast<std::uint32_t>(3u),
  ...
  test::AssertTrue(std::equal(decoded.metadata.segment_sample_counts,
  ...
  test::AssertEq(std::string(decoded.metadata.app_version.data,
                             decoded.metadata.app_version.size),
                 std::string(kTestAppVersion),
                 "C ABI metadata app version should round-trip.");
  test::AssertEq(std::string(decoded.metadata.core_version.data,
                             decoded.metadata.core_version.size),
...
void TestWavIoCApiMetadataMissingOnCanonicalWav() {
  ...
  const auto status = audio_io_decode_mono_pcm16_wav(
      wav_bytes.data(), wav_bytes.size(), &decoded);
  test::AssertEq(status, AUDIO_IO_WAV_OK,
                 "C ABI decode should still succeed for canonical WAV.");
  test::AssertEq(decoded.metadata_status, AUDIO_IO_METADATA_NOT_FOUND,
                 "C ABI unified decode should report metadata-not-found on "
                 "canonical WAV.");
  test::AssertContains(
      audio_io_metadata_status_message(decoded.metadata_status), "not found",
      "C ABI metadata status message should explain the missing-metadata "
      "case.");
  audio_io_free_decoded_wav(&decoded);
}
...
void TestWavIoCApiTruncatedWavPreservesMetadataStatus() {
  ...
  const auto status = audio_io_decode_mono_pcm16_wav(
      wav_bytes.data(), wav_bytes.size(), &decoded);
  test::AssertEq(status, AUDIO_IO_WAV_TRUNCATED_DATA,
                 "C ABI unified decode should return truncated WAV status for "
                 "truncated payloads.");
  test::AssertEq(decoded.metadata_status, AUDIO_IO_METADATA_OK,
                 "C ABI unified decode should preserve metadata success when "
                 "WBAG is intact.");
  test::AssertEq(decoded.metadata.mode, AUDIO_IO_METADATA_MODE_MINI,
...
void TestWavIoMetadataRoundTripContract() {
  ...
  const auto metadata =
      MakeValidMetadata(static_cast<std::uint32_t>(test_case.mono_pcm.size()));
  const auto wav_bytes = audio_io::SerializeMonoPcm16WavWithMetadata(
      test_case.sample_rate_hz, test_case.mono_pcm, metadata);
  test::AssertTrue(!wav_bytes.empty(),
                   "Metadata WAV serialization should succeed.");

  const auto parsed_wav = audio_io::ParseMonoPcm16Wav(wav_bytes);
  test::AssertEq(parsed_wav.status, audio_io::WavPcm16Status::kOk,
                 "PCM parsing should still succeed.");
  test::AssertAudioIoRoundTripResult(parsed_wav.wav, test_case,
                                     "Metadata WAV PCM round trip");
  ...
  test::AssertEq(parsed_metadata.status,
                 ...
                 "Metadata parse should succeed.");
  test::AssertEq(parsed_metadata.metadata.version, 7u,
                 "Metadata version should round-trip.");
  test::AssertEq(parsed_metadata.metadata.mode,
                 audio_io::FlipBitsAudioMetadataMode::kMini,
                 "Metadata mode should round-trip.");
  test::AssertTrue(
      !parsed_metadata.metadata.has_flash_voicing_style,
...
void TestWavIoMetadataModeValuesRoundTrip() {
  ...
  for (const auto mode : modes) {
    ...
    metadata.has_flash_voicing_style =
        mode == audio_io::FlipBitsAudioMetadataMode::kFlash;
    metadata.flash_voicing_style =
        metadata.has_flash_voicing_style
            ? audio_io::FlipBitsAudioMetadataFlashVoicingStyle::kHostility
            : audio_io::FlipBitsAudioMetadataFlashVoicingStyle::kUnknown;
    metadata.has_mini_speed_style =
        mode == audio_io::FlipBitsAudioMetadataMode::kMini;
    metadata.mini_speed_style =
        metadata.has_mini_speed_style
            ? audio_io::FlipBitsAudioMetadataMiniSpeedStyle::kFast
            : audio_io::FlipBitsAudioMetadataMiniSpeedStyle::kUnknown;
    ...
    test::AssertTrue(
        !wav_bytes.empty(),
        "Metadata WAV serialization should accept all transport modes.");
    const auto parsed_metadata =
        audio_io::ParseFlipBitsAudioMetadata(wav_bytes);
    test::AssertEq(parsed_metadata.status,
                   audio_io::FlipBitsAudioMetadataStatus::kOk,
                   "Metadata parsing should accept all transport modes.");
    test::AssertEq(parsed_metadata.metadata.mode, mode,
...
void TestWavIoMetadataMissingOnCanonicalWav() {
  ...
  const auto parsed_metadata = audio_io::ParseFlipBitsAudioMetadata(wav_bytes);
  test::AssertEq(
      parsed_metadata.status, audio_io::FlipBitsAudioMetadataStatus::kNotFound,
...
void TestWavIoMetadataRespectsChunkPadding() {
  ...
  padded_bytes.insert(padded_bytes.end(), wav_bytes.begin(),
                      wav_bytes.begin() + 36);
  ...
  padded_bytes.insert(padded_bytes.end(), wav_bytes.begin() + 36,
                      wav_bytes.end());
  ...
  test::AssertAudioIoRoundTripResult(parsed.wav, test_case,
                                     "Chunk padding WAV PCM round trip");
}
...
void TestWavIoMetadataRejectsOlderVersions() {
  ...
  const auto wav_bytes_v2 = audio_io::SerializeMonoPcm16WavWithMetadata(
      test_case.sample_rate_hz, test_case.mono_pcm, metadata_v2);
  test::AssertTrue(wav_bytes_v2.empty(),
                   "Serialization should reject older metadata versions now "
                   "that only v7 is writable.");
}
...
void TestWavIoMetadataParseRejectsUnsupportedVersion() {
  ...
  test::AssertEq(parsed_metadata.status,
                 ...
                 "Metadata parsing should surface unsupported versions from "
                 "existing WBAG chunks.");
}
...
void TestWavIoMetadataRejectsSegmentBoundariesThatDoNotMatchPcm() {
  ...
  const auto wav_bytes = audio_io::SerializeMonoPcm16WavWithMetadata(
      test_case.sample_rate_hz, test_case.mono_pcm, metadata);
  test::AssertTrue(wav_bytes.empty(),
                   "Serialization should reject segment boundaries that do not "
                   "add up to the PCM sample count.");
}
...
void TestWavIoMetadataFlashEmotionValuesRoundTrip() {
  ...
  for (const auto style : styles) {
    auto metadata = MakeValidFlashMetadata(
        static_cast<std::uint32_t>(test_case.mono_pcm.size()));
    ...
    test::AssertTrue(
        !wav_bytes.empty(),
        "Metadata WAV serialization should accept flash emotion values.");
    const auto parsed_metadata =
        audio_io::ParseFlipBitsAudioMetadata(wav_bytes);
    test::AssertEq(parsed_metadata.status,
                   audio_io::FlipBitsAudioMetadataStatus::kOk,
                   "Metadata parsing should accept flash emotion values.");
    test::AssertEq(parsed_metadata.metadata.flash_voicing_style, style,
...
void TestWavIoMetadataRejectsCorruptedFields() {
...

  {
    ...
    test::AssertEq(parsed_metadata.status,
                   audio_io::FlipBitsAudioMetadataStatus::kInvalidMetadata,
                   "Metadata parsing should reject empty app-version fields "
                   "inside WBAG chunks.");
  }
...
int main() {
  test::Runner runner;
  runner.Add("Unit.WavIoHeaderRoundTripContract",
             TestWavIoHeaderRoundTripContract);
  runner.Add("Unit.WavIoHeaderBytesRoundTripContract",
             TestWavIoHeaderBytesRoundTripContract);
  runner.Add("Unit.WavIoHeaderProbeRoundTripContract",
             TestWavIoHeaderProbeRoundTripContract);
  runner.Add("Unit.WavIoHeaderReadMissingFileFails",
             TestWavIoHeaderReadMissingFileFails);
  runner.Add("Unit.WavIoHeaderRejectsInvalidBytes",
             TestWavIoHeaderRejectsInvalidBytes);
  runner.Add("Unit.WavIoHeaderRejectsUnsupportedStereoBytes",
             TestWavIoHeaderRejectsUnsupportedStereoBytes);
  runner.Add("Unit.WavIoHeaderRejectsTruncatedDataBytes",
             TestWavIoHeaderRejectsTruncatedDataBytes);
  runner.Add("Unit.WavIoCApiRoundTripContract", TestWavIoCApiRoundTripContract);
  runner.Add("Unit.WavIoCApiRejectsInvalidBytes",
             TestWavIoCApiRejectsInvalidBytes);
  runner.Add("Unit.WavIoCApiProbeRoundTripContract",
             TestWavIoCApiProbeRoundTripContract);
  runner.Add("Unit.WavIoCApiMetadataRoundTripContract",
             TestWavIoCApiMetadataRoundTripContract);
  runner.Add("Unit.WavIoCApiMetadataMissingOnCanonicalWav",
             TestWavIoCApiMetadataMissingOnCanonicalWav);
  runner.Add("Unit.WavIoCApiTruncatedWavPreservesMetadataStatus",
             TestWavIoCApiTruncatedWavPreservesMetadataStatus);
  runner.Add("Unit.WavIoCApiFreeFunctionsAreIdempotentOnZeroedBuffers",
             TestWavIoCApiFreeFunctionsAreIdempotentOnZeroedBuffers);
  runner.Add("Unit.WavIoMetadataRoundTripContract",
             TestWavIoMetadataRoundTripContract);
  runner.Add("Unit.WavIoMetadataModeValuesRoundTrip",
             TestWavIoMetadataModeValuesRoundTrip);
  runner.Add("Unit.WavIoMetadataMissingOnCanonicalWav",
             TestWavIoMetadataMissingOnCanonicalWav);
  runner.Add("Unit.WavIoMetadataRespectsChunkPadding",
             TestWavIoMetadataRespectsChunkPadding);
  runner.Add("Unit.WavIoMetadataRejectsOlderVersions",
             TestWavIoMetadataRejectsOlderVersions);
  runner.Add("Unit.WavIoMetadataParseRejectsUnsupportedVersion",
             TestWavIoMetadataParseRejectsUnsupportedVersion);
  runner.Add("Unit.WavIoMetadataRejectsTruncatedChunkData",
             TestWavIoMetadataRejectsTruncatedChunkData);
  runner.Add("Unit.WavIoMetadataRejectsShortChunkPayload",
             TestWavIoMetadataRejectsShortChunkPayload);
  runner.Add("Unit.WavIoMetadataRejectsUnknownModeValues",
             TestWavIoMetadataRejectsUnknownModeValues);
  runner.Add("Unit.WavIoMetadataRejectsUnknownFlashStyleValues",
             TestWavIoMetadataRejectsUnknownFlashStyleValues);
  runner.Add("Unit.WavIoMetadataFlashEmotionValuesRoundTrip",
             TestWavIoMetadataFlashEmotionValuesRoundTrip);
  runner.Add("Unit.WavIoMetadataRejectsCorruptedFields",
             TestWavIoMetadataRejectsCorruptedFields);
  runner.Add("Unit.WavIoMetadataRejectsSegmentBoundariesThatDoNotMatchPcm",
             TestWavIoMetadataRejectsSegmentBoundariesThatDoNotMatchPcm);
  return runner.Run();
}
