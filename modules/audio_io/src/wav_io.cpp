...
namespace audio_io {
namespace {
std::vector<int16_t> ToMono(const std::vector<int16_t>& interleaved, int channels) {
    if (channels <= 1) {
        return interleaved;
    }
...
WavPcm16 ReadMonoPcm16Wav(const std::filesystem::path& input_path) {
    ...
    WavPcm16 output{};
    output.sample_rate_hz = info.samplerate;
    output.mono_pcm = ToMono(interleaved, info.channels);
    return output;
}
