...
def is_cjk_language(lang_code: str) -> bool:
    ...
    return any(lang_code.startswith(prefix) for prefix in policy.detection.cjk_language_prefixes)
...
def should_skip_mixed_language_detection(lang_code: str) -> bool:
    ...
    """
    policy = load_mixed_language_policy()
    return lang_code in policy.detection.mixed_language_skip_codes
...
def _allowed_latin_words_for_lang(lang_code: str) -> set[str]:
    policy = load_mixed_language_policy()
    allowed = set(policy.detection.global_allowed_latin_words)
    allowed.update(policy.detection.per_language_allowed_latin_words.get(lang_code, frozenset()))
    return allowed


def check_cjk_language_for_latin_chunks(target_text: str, *, lang_code: str) -> list[str]:
    """
    针对中文、日文、韩文：
    ...
    suspicious: list[str] = []
    allowed_words = _allowed_latin_words_for_lang(lang_code)
    for chunk in chunks:
        chunk = chunk.strip()
        if not chunk:
            continue
        ...
        is_safe = all(word.lower() in allowed_words or len(word) == 1 for word in words)
        if not is_safe:
