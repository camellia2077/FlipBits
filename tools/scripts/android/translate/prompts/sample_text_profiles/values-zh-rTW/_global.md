# ZH-rTW Sample Global Guardrails

Apply these rules to all Traditional Chinese sample-text faction profiles under `values-zh-rTW`.

1. Semantic priority
- Preserve core meaning, proposition structure, and imagery from the source line.
- Natural Traditional Chinese adaptation is required; literal word order is not required.
...
- Avoid excessive modern internet slang or playful tone.
...
- Make edits only for high-impact issues:
  - register mismatch (for example respectful/polite register where the faction requires a different register)
